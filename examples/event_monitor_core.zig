...
pub const EventMonitor = struct {
    ...
    pub fn process(self: *EventMonitor, context: *plug.process.ProcessContext(f32)) void {
        ...
        if (context.latestEvent(.aftertouch)) |event| {
            if (event.asAftertouch()) |aftertouch| {
                self.latest_aftertouch_bus = aftertouch.bus_index;
            }
        }
        if (context.latestEvent(.note_expression_int)) |event| {
            if (event.asNoteExpressionInt()) |expression| {
                self.latest_note_expression_int_value = expression.value;
            }
        }
    }
};
...
test "event monitor core example summarizes input event kinds" {
    ...
    try std.testing.expect(!input_event_view.isEmpty());
    try std.testing.expect(input_event_view.hasEvents());
    try std.testing.expectEqual(@as(?usize, 1), input_event_view.firstSampleOffset());
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.latestSampleOffset());
    try std.testing.expectEqual(@as(?usize, 2), input_event_view.firstSampleOffsetForKind(.midi_cc));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.latestSampleOffsetForKind(.note_on));
    try std.testing.expectEqual(@as(?usize, 1), input_event_view.firstSampleOffsetForBus(0));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.latestSampleOffsetForBus(0));
    try std.testing.expectEqual(@as(?usize, 1), input_event_view.firstSampleOffsetForChannel(0));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.latestSampleOffsetForChannel(0));
    try std.testing.expectEqual(@as(?usize, 1), input_event_view.firstSampleOffsetForBusChannel(0, 0));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.latestSampleOffsetForBusChannel(0, 0));
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.first().?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_off, input_event_view.latest().?.kind);
    try std.testing.expectEqual(plug.process.EventKind.midi_cc, input_event_view.firstAtOffset(2).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_off, input_event_view.latestAtOffset(3).?.kind);
    try std.testing.expectEqual(@as(usize, 2), input_event_view.countKind(.note_on));
    try std.testing.expectEqual(@as(usize, 2), input_event_view.countNoteAttacks());
    try std.testing.expectEqual(@as(usize, 1), input_event_view.countNoteReleases());
    try std.testing.expectEqual(@as(usize, 9), input_event_view.countAtOffset(3));
    try std.testing.expectEqual(@as(usize, 1), input_event_view.countKindAtOffset(.note_on, 3));
    try std.testing.expectEqual(@as(usize, 10), input_event_view.countBus(0));
    try std.testing.expectEqual(@as(usize, 6), input_event_view.countChannel(0));
    try std.testing.expectEqual(@as(usize, 5), input_event_view.countBusChannel(0, 0));
    try std.testing.expect(input_event_view.hasBus(0));
    try std.testing.expect(!input_event_view.busEmpty(0));
    try std.testing.expect(input_event_view.busEmpty(3));
    try std.testing.expect(input_event_view.hasChannel(0));
    try std.testing.expect(input_event_view.channelEmpty(9));
    try std.testing.expect(input_event_view.hasBusChannel(0, 0));
    try std.testing.expect(input_event_view.busChannelEmpty(3, 0));
    try std.testing.expectEqual(plug.process.EventKind.midi_cc, input_event_view.firstKind(.midi_cc).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.latestKind(.note_on).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.firstKindAtOffset(.note_on, 3).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.latestKindAtOffset(.note_on, 3).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.firstBus(0).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_off, input_event_view.latestBus(0).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.firstChannel(0).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_off, input_event_view.latestChannel(0).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_on, input_event_view.firstBusChannel(0, 0).?.kind);
    try std.testing.expectEqual(plug.process.EventKind.note_off, input_event_view.latestBusChannel(0, 0).?.kind);
    try std.testing.expect(input_event_view.hasKind(.note_on));
    try std.testing.expect(input_event_view.hasNoteAttacks());
    try std.testing.expect(input_event_view.hasNoteReleases());
    try std.testing.expect(input_event_view.hasAtOffset(3));
    try std.testing.expect(input_event_view.hasKindAtOffset(.note_on, 3));
    try std.testing.expect(!input_event_view.kindEmpty(.note_on));
    try std.testing.expect(!input_event_view.noteAttacksEmpty());
    try std.testing.expect(!input_event_view.noteReleasesEmpty());
    try std.testing.expect(input_event_view.offsetEmpty(0));
    try std.testing.expect(input_event_view.kindAtOffsetEmpty(.midi_cc, 3));
    try std.testing.expect(!input_event_view.onlyAtOffset(3));
    try std.testing.expect(!input_event_view.onlyKindAtOffset(.note_on, 3));
    try std.testing.expect(!input_event_view.onlyKind(.note_on));
    try std.testing.expect(!input_event_view.onlyNoteAttacks());
    try std.testing.expect(!input_event_view.onlyNoteReleases());
    try std.testing.expect(!input_event_view.onlyBus(0));
    try std.testing.expect(!input_event_view.onlyChannel(0));
    try std.testing.expect(!input_event_view.onlyBusChannel(0, 0));
    try std.testing.expectEqual(@as(?usize, 2), input_event_view.nextSampleOffset(1));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.nextSampleOffsetForKind(.note_on, 1));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.nextSampleOffsetForBus(2, 0));
    try std.testing.expectEqual(@as(?usize, 2), input_event_view.nextSampleOffsetForChannel(0, 1));
    try std.testing.expectEqual(@as(?usize, 3), input_event_view.nextSampleOffsetForBusChannel(0, 0, 2));
    var note_on_events = input_event_view.ofKind(.note_on);
    try std.testing.expectEqual(@as(usize, 1), note_on_events.next().?.sample_offset);
    try std.testing.expectEqual(@as(usize, 3), note_on_events.next().?.sample_offset);
    try std.testing.expect(note_on_events.next() == null);
    ...
    try std.testing.expectEqual(
        ...
        input_event_segments.next().?,
    );
    try std.testing.expectEqual(
        plug.process.BlockSegment{ .start_offset = 1, .end_offset = 2 },
        input_event_segments.next().?,
    );
