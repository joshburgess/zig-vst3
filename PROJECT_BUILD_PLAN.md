...
The parameter layer now includes reflected float, int, bool, and enum descriptors, numeric plain-range helpers, reflected optional numeric-range metadata, enum option introspection, normalized/plain conversion, text formatting/parsing, atomic value storage, modulation offsets, linear/exponential/logarithmic smoothing helpers with completion and coefficient reads, parameter views/editors, count and emptiness predicates, and instance-level wrappers with coverage for invalid metadata, missing metadata, and reset paths.
