...
**Current status.** Basic VST3 input and output event translation is implemented. `process.ProcessContext` exposes note-on, note-off, MIDI CC, pitch bend, aftertouch, note-expression value/int/text, data payloads such as SysEx, and other event kinds with block-offset validation, per-kind iteration, per-kind first/latest/next offset helpers, block-wide segments split at every input-event offset, and combined process segments split at input-event and parameter-change offsets. The reusable shell advertises an input event bus, can opt into an output event bus, collects bounded event lists during `process`, gives processors a bounded output-event writer with capacity checks, and flushes written events to the host. The pure `note_gate_core` example and bundled `zig_vst3_note_gate` plugin exercise event consumption. The pure `event_echo_core` example and bundled `zig_vst3_event_echo` plugin exercise event production, `event_monitor_core` exercises event-kind summaries for MIDI-style code, `sine_synth_core` exercises event-offset synthesis with sample-accurate level automation, and the bundled event echo validates locally on macOS with one input and one output event bus.
