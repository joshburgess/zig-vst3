...
**Current status.** The first prototype exists under `zig-plug/src/` and is also exposed as the pure `zig-plug-core` module for Layer 1 integration. The reusable VST3 bridge now covers reflected controller metadata, string conversion, normalized/plain conversion, parameter controller/state helpers, host automation collection and application, state serialization, stereo audio bus metadata, and process context construction. The gain plugin uses that bridge while still owning its concrete component/controller shell. `examples/gain_core.zig`, `examples/bypass_core.zig`, `examples/mode_gain_core.zig`, and `examples/voice_mix_core.zig` are checked pure-API examples covering float, bool, enum, and int parameters. The remaining Phase 5 work is to replace the gain-specific VST3 shell with a reusable wrapper, then add bundled VST3 examples.
