...
**Current status.** Basic VST3 input and output event translation is implemented. `process.ProcessContext` exposes note-on, note-off, MIDI CC, pitch bend, aftertouch, note-expression value/int/text, bounded data payloads such as SysEx, and other event kinds with typed payload views, typed note attack/release views, block-offset validation, kind/offset predicates, note attack/release predicates, event category predicates, bus and channel predicates, per-kind iteration, per-kind/bus/channel/bus-channel first/latest/next offset helpers, kind/bus/channel/offset emptiness and only-event predicates, audio channel presence and emptiness helpers, block-wide segments split at every input-event offset, and combined process segments split at input-event and parameter-change offsets. The reusable shell advertises an input event bus, can opt into an output event bus, collects bounded event lists during `process`, drops malformed or oversized host data events, gives processors a bounded output-event writer with capacity checks and output-event query helpers, and flushes written events to the host. The pure `note_gate_core` example and bundled `zig_vst3_note_gate` plugin exercise event consumption with sample-offset gating, channel-aware overlapping held notes, and zero-velocity note-on release behavior. The pure `event_echo_core` example and bundled `zig_vst3_event_echo` plugin exercise event production, `event_monitor_core` exercises event-kind summaries and typed MIDI-style payload reads, and the pure and bundled sine-synth examples exercise event-offset synthesis with sample-accurate level automation. The bundled event echo validates locally on macOS with one input and one output event bus.
