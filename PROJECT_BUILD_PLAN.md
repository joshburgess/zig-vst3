...
**Current status.** Layer 1 now builds reusable VST3 effect, input-only analyzer, and output-only generator shells, emits `.vst3` bundles for macOS, Linux, and Windows, and validates the bundled gain, bypass, mode-gain, voice-mix, note-gate, event-echo, event-monitor, and sine-synth examples on macOS and Linux in public CI. CI also runs Linux/macOS/Windows build and test jobs, Linux/macOS Layer 1 ABI checks, and Linux/macOS/Windows cross-bundle smoke checks. The reusable shells expose conservative default `IConnectionPoint` implementations for host/plugin connection setup, retain and release host callback interfaces across replacement and repeated initialization, reset stateful processors during setup, deactivation, and processing stop when a reset hook is provided, clear fixed-size VST string output buffers before writing, harden fixed-capacity stream, message, attribute, parameter-change, event-list, unit-info, component-handler, host-context, inter-app, plug-frame, compatibility JSON, capability-support, process bridge, audio-processor helper, bypass-delay helper, speaker-array, factory count, note-expression value, and MIDI conversion helpers against stale outputs, corrupted counts, overflow indices, invalid values, invalid ranges, and oversized ABI casts, and expose an optional edit-controller plug-view factory hook for plugin-provided editors.
...
- `zig build validate-examples` passes locally on macOS and in CI on macOS and Linux.
- `zig build phase1` runs the bundled example validator path on macOS and Linux.
...
State serialization streams. The raw error-context, string-result, cloneable, update-handler, persistence, and plugin-compatibility interfaces are translated. `vst_stream.zig` provides a reusable fixed-buffer `IBStream`/`ISizeableStream` object for state, preset, and SDK callback tests with deterministic failed read/write counts and explicit seek-mode validation, `vst_error_context.zig` provides a reusable fixed-message `IErrorContext`, `vst_string_result.zig` provides a reusable fixed-buffer `IStringResult`/`IString` object for SDK callbacks that return text, `vst_cloneable.zig` provides a reusable hook-based `ICloneable` adapter, `vst_update_handler.zig` provides reusable fixed-slot `IUpdateHandler`/`IDependent` objects for dependency notifications, `vst_persistent_attributes.zig` provides a reusable fixed-slot `IAttributes`/`IAttributes2` store for variant and binary metadata with deterministic binary read buffers, and `vst_plugin_compatibility.zig` provides a reusable JSON compatibility provider.
...
`IComponentHandler`, `IComponentHandler2`, `IComponentHandler3`, `IHostApplication`, `IInfoListener`, `IAutomationState`, `IDataExchangeHandler`, `IDataExchangeReceiver`, `IConnectionPoint`, `IMessage`, `IAttributeList`, `IStreamAttributes`, `IXmlRepresentationController`. The reusable component and controller shells now query and retain `IHostApplication`, retain host channel-context, automation-state, and data-exchange handler interfaces, expose default `IConnectionPoint` and `IDataExchangeReceiver` objects, expose an optional XML representation hook, reset stateful processors on host lifecycle stop points when configured, and store `IComponentHandler`/`IComponentHandler2`/`IComponentHandler3` plus bus-activation, system-time, `IUnitHandler`, and `IUnitHandler2` handlers for edit, editor, dirty-state, group-edit, context-menu, bus, host-time, unit, and program-list callbacks. `vst_component_handler.zig` provides reusable component-handler, bus-activation, system-time, progress, and unit-handler helpers with deterministic delegated failure outputs. `vst_host_application.zig` provides reusable `IHostApplication`, wrapper-marker, and wrapper MPE-support objects for host-side tests with deterministic failed factory outputs. `vst_host_context.zig` provides reusable host applications with `IInfoListener`, `IAutomationState`, and `IDataExchangeHandler` for channel-context, automation-state, and data-exchange tests with deterministic failed factory, queue, and block-lock outputs. `vst_capability_support.zig` provides reusable interface-support, prefetch-state, MIDI learn, and physical UI mapping helpers. `vst_message.zig` provides reusable `IMessage`, fixed-capacity `IAttributeList`, and stream-attribute objects with deterministic binary attribute storage. `vst_context_menu.zig` provides reusable fixed-capacity `IContextMenu` and hook-based `IContextMenuTarget` objects. `vst_persistent_attributes.zig` provides reusable `IPersistent`, `IAttributes`, and `IAttributes2` objects.

### Work Unit 2.9: Plug view (no GUI implementation yet)

`IPlugView`, `IPlugFrame`, `IPlugViewContentScaleSupport`, `IParameterFinder`, `ViewRect`, platform-specific `kPlatformType*` constants. The raw ABI translation is present, reflected edit controllers can delegate `createView` to a plugin config hook, `vst_plug_frame.zig` provides a reusable resize-tracking `IPlugFrame`, `vst_parameter_finder.zig` provides a reusable coordinate-to-parameter object, `vst_content_scale_support.zig` provides reusable positive scale-factor handling for custom editors, and `vst_linux_run_loop.zig` provides fixed-slot Linux `IRunLoop`/event/timer handler utilities. There is still no bundled GUI toolkit.

### Work Unit 2.9a: Linux Wayland plug view interfaces

`IWaylandHost`, `IWaylandFrame`, and `kPlatformTypeWaylandSurfaceID` from SDK 3.8.0. The raw ABI translation is present, and `vst_wayland_frame.zig` provides reusable null-default `IWaylandHost` and `IWaylandFrame` objects for custom Linux editors and host-side tests.
...
`IInterAppAudioHost`, `IInterAppAudioConnectionNotification`, `IInterAppAudioPresetManager`, `ITestPlugProvider`, `ITestPlugProvider2`. These are P3 legacy/mobile/test-support interfaces. `vst_inter_app_audio.zig` provides reusable hook-based `IInterAppAudioHost`, stateful `IInterAppAudioConnectionNotification`, and hook-based `IInterAppAudioPresetManager` objects with deterministic failed screen-size and host-icon outputs. `vst_test_plug_provider.zig` provides a reusable `ITestPlugProvider`/`ITestPlugProvider2` object for local validator and host-test harnesses.
...
**Current status.** Basic VST3 input and output event translation is implemented. `process.ProcessContext` exposes note-on, note-off, MIDI CC, pitch bend, aftertouch, note-expression value/int/text, bounded data payloads such as SysEx, and other event kinds with typed payload views, typed note attack/release views, block-offset validation, kind/offset predicates, note attack/release predicates, event category predicates, bus and channel predicates, per-kind iteration, per-kind/bus/channel/bus-channel first/latest/next offset helpers, kind/bus/channel/offset emptiness and only-event predicates, audio channel presence and emptiness helpers, block-wide segments split at every input-event offset, and combined process segments split at input-event and parameter-change offsets. The reusable shell advertises an input event bus, can opt into an output event bus, collects bounded event lists during `process`, drops malformed or oversized host data events, gives processors a bounded output-event writer with capacity checks and output-event query helpers, and flushes written events to the host. The pure `note_gate_core` example and bundled `zig_vst3_note_gate` plugin exercise event consumption with sample-offset gating, channel-aware overlapping held notes, and zero-velocity note-on release behavior. The pure `event_echo_core` example and bundled `zig_vst3_event_echo` plugin exercise event production, `event_monitor_core` exercises event-kind summaries, routing offset helpers, and typed MIDI-style payload reads, and the pure and bundled sine-synth examples exercise event-offset synthesis with sample-accurate level automation. The bundled event echo validates locally on macOS with one input and one output event bus.
...
- Bundled VST3 examples for gain, bypass, mode gain, voice mix, note gate, event echo, and sine synth
