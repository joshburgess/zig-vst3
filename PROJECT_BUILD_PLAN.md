...
`IComponentHandler`, `IComponentHandler2`, `IComponentHandler3`, `IHostApplication`, `IInfoListener`, `IAutomationState`, `IDataExchangeHandler`, `IDataExchangeReceiver`, `IConnectionPoint`, `IMessage`, `IAttributeList`, `IStreamAttributes`, `IXmlRepresentationController`. The reusable component and controller shells now query and retain `IHostApplication`, retain host channel-context, automation-state, and data-exchange handler interfaces, expose default `IConnectionPoint` and `IDataExchangeReceiver` objects, expose an optional XML representation hook, and store `IComponentHandler`/`IComponentHandler2`/`IComponentHandler3` plus bus-activation, system-time, `IUnitHandler`, and `IUnitHandler2` handlers for edit, editor, dirty-state, group-edit, context-menu, bus, host-time, unit, and program-list callbacks. `vst_component_handler.zig` provides a reusable `IComponentHandler` for controller automation callback tests. `vst_host_application.zig` provides reusable `IHostApplication`, wrapper-marker, and wrapper MPE-support objects for host-side tests. `vst_host_context.zig` provides reusable host applications with `IInfoListener`, `IAutomationState`, and `IDataExchangeHandler` for channel-context, automation-state, and data-exchange tests. `vst_message.zig` provides reusable `IMessage`, fixed-capacity `IAttributeList`, and stream-attribute objects with deterministic binary attribute storage. `vst_context_menu.zig` provides reusable fixed-capacity `IContextMenu` and hook-based `IContextMenuTarget` objects. `vst_persistent_attributes.zig` provides reusable `IPersistent`, `IAttributes`, and `IAttributes2` objects.
...
`IUnitInfo`, `IProgramListData`, `IUnitData`, `UnitInfo`, `ProgramListInfo`. `vst_unit_data.zig` provides reusable fixed-slot `IUnitInfo` and `IProgramListData`/`IUnitData` objects for unit, program-list, and stream callbacks.
