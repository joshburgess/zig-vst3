...
**Current status.** Basic VST3 input and output event translation is implemented. `process.ProcessContext` exposes note-on, note-off, MIDI CC, pitch bend, aftertouch, note-expression value/int/text, data, and other event kinds with block-offset validation. The reusable shell advertises one input event bus, collects bounded event lists during `process`, gives processors a bounded output-event writer, and flushes written events to the host. The pure `note_gate_core` example and bundled `zig_vst3_note_gate` plugin exercise event consumption, and the bundled plugin passes Steinberg's validator locally on macOS.
