...
- Decode complete MPEG Layer III frames through gapless stream ranges. Emit checked CBR and VBR MPEG-1, MPEG-2, and MPEG-2.5 frames with exact padding cadence, optional CRC protection, all pair and count1 Huffman codebooks, mid-side or intensity stereo, stateful tonal and temporal masking, bounded main-data reuse for either rate policy, gapless Info or Xing metadata, caller-planned VBRI metadata, composed ID3v2 prefixes and ID3v1 tails, and recoverable memory or file output.
