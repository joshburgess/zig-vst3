...
- `zig-plug` float, int, bool, and enum parameter descriptors with normalized/plain conversion, display formatting, parsing, atomic value storage, smoothing helpers, and reflected parameter-change construction
- `zig-plug` plugin specs and instances with reflected defaults, lifecycle validation, typed parameter access, bound parameter views, parameter state read/write, and process dispatch for 32-bit and 64-bit audio
- `zig-plug` process context helpers for typed audio buffers, sample-accurate parameter changes, input events, output events, and attachment-based context construction
