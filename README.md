...
- Reusable `IMessage`, `IErrorContext`, `IStringResult`, `IString`, `ICloneable`, `IPersistent`, `IAttributes`, `IAttributes2`, `IUpdateHandler`, `IDependent`, `IAttributeList`, `IStreamAttributes`, `IInterAppAudioConnectionNotification`, and `IInterAppAudioPresetManager` utility objects for host/plugin notifications, error reporting, dependency updates, cloning adapters, and state/preset metadata
