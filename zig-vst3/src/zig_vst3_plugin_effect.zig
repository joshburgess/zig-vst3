...
const effect_support = @import("effect_support.zig");
...
const zig_vst3_edit_controller = @import("zig_vst3_edit_controller.zig");
...
pub const ParameterObserver = zig_vst3_edit_controller.ParameterObserver;
...
pub const ReflectedEditController =
    zig_vst3_edit_controller.ReflectedEditController;

test "VST3 effect facade preserves exact controller identities" {
    const EmptyParams = struct {};
    const ParameterSet = plug_core.parameters.ParameterSet(EmptyParams);
    const Config = struct {
        pub const controller_name = "IdentityController";
        pub const Params = EmptyParams;
        pub const parameter_set = &ParameterSet.init(.{});
    };

    try std.testing.expect(
        ParameterObserver == zig_vst3_edit_controller.ParameterObserver,
    );
    try std.testing.expect(
        ReflectedEditController(Config) ==
            zig_vst3_edit_controller.ReflectedEditController(Config),
    );
}
...
const queryHostApplication = effect_support.queryHostApplication;
const releaseHostApplication = effect_support.releaseHostApplication;
const queryInfoListener = effect_support.queryInfoListener;
const releaseInfoListener = effect_support.releaseInfoListener;
const queryAutomationState = effect_support.queryAutomationState;
const releaseAutomationState = effect_support.releaseAutomationState;
const queryDataExchangeHandler = effect_support.queryDataExchangeHandler;
const releaseDataExchangeHandler = effect_support.releaseDataExchangeHandler;
const retainConnectionPeer = effect_support.retainConnectionPeer;
const releaseConnectionPeer = effect_support.releaseConnectionPeer;
const replaceConnectionPeer = effect_support.replaceConnectionPeer;
const disconnectConnectionPeer = effect_support.disconnectConnectionPeer;
const failOpenedDataExchangeQueue = effect_support.failOpenedDataExchangeQueue;
const failLockedDataExchangeBlock = effect_support.failLockedDataExchangeBlock;
