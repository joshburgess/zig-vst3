...
pub fn build(b: *std.Build) void {
    ...
    const bundle = b.step("bundle", "Build the native downstream effect bundle");
    addNativeBundle(b, bundle, target, library);

    const test_step = b.step("test", "Test the downstream effect and build its bundle");
...
fn addNativeBundle(
    ...
    bundle.dependOn(&b.addInstallFile(
        b.path("src/assets/default-preset.txt"),
        "bundle/DownstreamEffect.vst3/Contents/Resources/default-preset.txt",
    ).step);
    if (target.result.os.tag == .macos) {
        bundle.dependOn(&b.addInstallFile(
            b.path("bundle/Info.plist"),
            "bundle/DownstreamEffect.vst3/Contents/Info.plist",
        ).step);
        bundle.dependOn(&b.addInstallFile(
            b.path("bundle/PkgInfo"),
            "bundle/DownstreamEffect.vst3/Contents/PkgInfo",
        ).step);
    }
}
