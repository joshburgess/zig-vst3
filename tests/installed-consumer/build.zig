...
pub fn build(b: *std.Build) void {
    ...
    const public_api_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("public_api.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    public_api_tests.root_module.addImport(
        "zig-vst3-plugin",
    ...
    const framework_api_compatibility_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("framework_api_compatibility.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    framework_api_compatibility_tests.root_module.addImport(
        "zig-vst3-plugin",
        dependency.module("zig-vst3-plugin"),
    );
    framework_api_compatibility_tests.root_module.addImport(
    ...
    test_step.dependOn(&b.addRunArtifact(framework_api_compatibility_tests).step);
