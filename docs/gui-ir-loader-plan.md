...
- Loading all eight GUI examples into one REAPER process exposed cross-bundle symbol interposition. The host logged duplicate `ZigVstguiAccessibilityElement` registrations and then crashed in a VSTGUI draw callback from a different bundle. VSTGUI and adapter C++ and Objective-C++ symbols are now hidden per bundle, and the accessibility bridge creates an image-unique runtime subclass. Entry-symbol validation rejects exported VSTGUI implementation symbols and fixed accessibility class names. The multi-plugin REAPER audit must pass before host validation is complete.
