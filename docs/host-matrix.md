...
| Deferred | REAPER | Pending | Pending | Pending | Pending | `zig_vst3_sidechain_ducker.vst3` | Deferred | Needs a real-host auxiliary-input routing test with stereo program audio and a mono key signal. |
| Deferred | REAPER | Pending | Pending | Pending | Pending | `zig_vst3_aux_output_splitter.vst3` | Deferred | Needs independent routing and audible confirmation of the stereo main output, mono auxiliary mix, and stereo auxiliary copy. |
...
5. Exercise the plugin-specific behavior:
   ...
   - `zig_vst3_sidechain_ducker.vst3`: route a mono key signal to the auxiliary input and confirm it attenuates the stereo program input.
   - `zig_vst3_aux_output_splitter.vst3`: route all three output buses and confirm stereo pass-through on the main output, a mono downmix on the first auxiliary output, and a stereo copy on the second.
...
- `zig_vst3_sidechain_ducker.vst3`: create a four-channel track or equivalent sidechain route, send stereo program audio to the main input and a mono key to the auxiliary input, then confirm the Depth parameter changes attenuation.
- `zig_vst3_aux_output_splitter.vst3`: expose all output pins, route each to a separate destination, and confirm the main stereo copy, mono downmix, and second stereo copy remain distinct.
