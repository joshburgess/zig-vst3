...
The checked gain example covers direct state size constants, checked encoded-size helpers, header-only writing, header entry-count, empty, delta, and encoded-size helpers, plus migration diagnostic indexes, in addition to instance-bound compatibility checks.
