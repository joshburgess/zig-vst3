...
Checked output-event coverage includes direct writer and process-context append planning, append counts, capacity state, kind emptiness, offset and kind-at-offset predicates, query views, block segments, no-writer fallbacks, validation failures, and clearing.
