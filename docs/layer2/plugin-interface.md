...
`PrepareConfig.validate` and `PluginInstance.prepareChecked` reject non-positive or non-finite sample rates and zero maximum block sizes before a plugin's `prepare` hook runs. `PluginInstance.prepare` remains the convenience wrapper for callers that already trust the configuration.
