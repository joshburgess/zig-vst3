...
Checked input-event helper coverage includes direct event validation, event classification, typed payload views, event-view validation, count, emptiness, first/latest, next-offset, iterator, block segment, routing, only predicates, and empty input fallbacks.
