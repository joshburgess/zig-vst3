...
- `ProcessContext.setOutputEvents`, `appendOutputEvent`, `appendOutputEvents`, `writtenOutputEvents`, `clearOutputEvents`, output-event kind and first/latest/next per-kind offset reads, `outputEventCount`, `outputEventCapacity`, `outputEventRemainingCapacity`, `outputEventsEmpty`, and `outputEventsFull` let processors write, inspect, reset, and plan output events without unwrapping the optional writer. Output-event writers must use the same frame count as the process context.
