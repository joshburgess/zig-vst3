...
- `ProcessContext.inputEvents`, `inputEventsOfKind`, `inputEventsAtOffset`, `inputEventBlockSegments`, `inputEventCount`, input-event presence and emptiness checks, first/latest/next event offsets, per-kind first/latest/next offsets, `firstEvent`, `latestEvent`, `hasEvent`, `countEvents`, per-offset and kind-at-offset event count/presence/emptiness checks, and bus/channel/bus-channel event presence, emptiness, count, and only-event helpers expose input-event reads without reaching into the event view field.
- `EventWriter.appendAll` copies validated event views into bounded output storage. Its count, empty/full, capacity, remaining-capacity, frame-count, `canAppend`, written-event view, block segment, offset, kind-query, per-offset and kind-at-offset queries, and bus/channel/bus-channel presence, emptiness, count, and only-event helpers support direct tests and non-context adapters.
- `ProcessContext.setOutputEvents`, `appendOutputEvent`, `appendOutputEvents`, `canAppendOutputEvent`, `canAppendOutputEvents`, `writtenOutputEvents`, `outputEventsOfKind`, `clearOutputEvents`, output-event kind and first/latest/next per-kind offset reads, output-event bus/channel/bus-channel presence, emptiness, count, per-offset and kind-at-offset helpers, and only-event helpers, `outputEventCount`, `outputEventCapacity`, `outputEventRemainingCapacity`, `outputEventFrameCount`, output-event presence and emptiness checks, and `outputEventsFull` let processors write, inspect, reset, and plan output events without unwrapping the optional writer. Output-event writers must use the same frame count as the process context.
