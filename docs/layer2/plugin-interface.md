...
`PrepareConfig.validate` and `PluginInstance.prepareChecked` reject non-positive or non-finite sample rates and zero maximum block sizes before a plugin's `prepare` hook runs. `PluginInstance.prepare` remains the convenience wrapper for callers that already trust the configuration. The VST3 shell applies the same checks to host process setup before accepting it.
