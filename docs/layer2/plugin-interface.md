...
- `ProcessContext.inputEvents`, `inputEventCount`, `inputEventsEmpty`, first/latest/next event offsets, per-kind next offsets, `firstEvent`, `latestEvent`, `hasEvent`, and `countEvents` expose input-event reads without reaching into the event view field.
