...
- `ParameterSet`, `ParameterView`, `ParameterEditor`, and `PluginInstance` expose parameter metadata by index, id, and display name for ids, short names, units, defaults, bypass flags, automation/read-only flags, unit ids, step counts, and list flags, plus id and display-name existence checks.
