...
- `ParameterValues`, `ParameterView`, `ParameterEditor`, and `PluginInstance` expose default-state checks by index, id, display name, and comptime field name, plus aggregate all-defaults, non-default presence, and non-default count helpers for reset buttons and dirty-state UI.
