...
- `AudioInputs(Sample)` and `AudioOutputs(Sample)` validate channel frame counts and expose channel access, channel count, emptiness, presence, frame-count, and output fill/clear helpers.
...
- `inputAudio`, `outputAudio`, `inputChannel`, `outputChannel`, input/output per-index channel presence and emptiness checks, `inputChannelCount`, `outputChannelCount`, input/output channel presence and emptiness checks, `inputFrameCount`, `outputFrameCount`, and `frameCount`
...
- `ParameterChanges.init` validates process-block changes and exposes value-level count, emptiness, first/latest, per-id, per-offset, only-match, normalized/defaulted-normalized, at-or-before, next-offset, and segment helpers.
