...
The checked gain example covers direct `ParameterValues` storage, copying, default-state helpers, counted stores/resets, bound `ParameterView` metadata and reads, bound `ParameterEditor` metadata and edits, and editor-level process-change application counts.
