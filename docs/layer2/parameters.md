...
The checked gain example covers direct `FloatParam` descriptor utilities, direct `ParameterSet` conversion helpers, direct `ParameterValues` storage, instance-bound parameter handles, copying, default-state helpers, counted stores/resets, reset aliases, direct field descriptor, flag, unit, option, and range metadata, bound `ParameterView` metadata, conversions, and reads, bound `ParameterEditor` metadata, conversions, and edits, and editor-level process-change application counts.
