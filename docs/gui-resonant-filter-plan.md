...
- A final direct pluginval strictness 5 launch stopped on Gain with exit 134. The macOS crash report showed pluginval aborting in AppKit application registration before any plugin image loaded. Comparison with successful runs isolated the problem to launching the application bundle's executable outside the logged-in GUI bootstrap session. The runner now uses `launchctl asuser` for a macOS `.app`, retains headless behavior and the real pluginval exit status, and resolves relative plugin paths before launch. A controlled Gain run passed, followed by complete serialized strictness 5 and 10 matrices. Each matrix passed all 13 plugins in 56/56 steps without a crash. The final Resonant Filter artifacts are preserved at `/var/folders/2r/700z0d517dg3yqy2_px199p00000gn/T/zig-vst3-pluginval/zig_vst3_resonant_filter-strictness-5-20260720-101628-20510` and `/var/folders/2r/700z0d517dg3yqy2_px199p00000gn/T/zig-vst3-pluginval/zig_vst3_resonant_filter-strictness-10-20260720-102038-27298`.
