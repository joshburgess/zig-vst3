...
| `IAudioProcessor`, `IAudioPresentationLatency`, `IProcessContextRequirements`, and audio processor buffer helpers | Initial vtable, struct, and helper translation complete; layout checked against the pinned SDK with `zig build audio-processor-abi` |
...
| Speaker constants, core speaker arrangements, `SpeakerArray`, and speaker helper functions | Initial constant and helper translation complete; values checked against the pinned SDK with `zig build speaker-core-abi` |
