...
| `IPlugInterfaceSupport`, `IPrefetchableSupport`, `IAutomationState` | Initial vtable translation complete; constants and IIDs checked with `zig build capability-state-abi`; reusable interface-support and prefetch-state helpers added in `vst_capability_support.zig`; reusable automation-state host helper added in `vst_host_context.zig` |
