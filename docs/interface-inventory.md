...
| `IProgress`, `IEditController2`, `IComponentHandler2`, `IComponentHandlerBusActivation`, `IEditControllerHostEditing`, `IComponentHandlerSystemTime` | Initial vtable translation complete; IIDs checked with `zig build edit-controller-abi`; reusable component-handler helpers cover automation callbacks, delegated failure propagation, editor/group callbacks, context-menu extension queries, bus activation, system time, progress callbacks, and unit-handler callbacks; dirty-state and bus-activation inputs are validated before recording or delegation |
