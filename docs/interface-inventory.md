...
| `IPlugView`, `IPlugFrame`, `IPlugViewContentScaleSupport`, and Linux run loop interfaces | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build plugview-abi`; reusable plug-view, plug-frame, and content-scale helpers preserve previous accepted state when delegated hooks reject a change, reject invalid scale factors, and track host-visible input, focus, frame, size, attachment, removal, and resize behavior; reusable Linux run-loop helper rejects null handlers and invalid file descriptors |
