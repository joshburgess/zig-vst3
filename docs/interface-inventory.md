...
| `IPlugView`, `IPlugFrame`, `IPlugViewContentScaleSupport`, and Linux run loop interfaces | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build plugview-abi`; reusable plug-frame and content-scale helpers preserve previous accepted state when delegated hooks reject a change; reusable Linux run-loop helper rejects null handlers and invalid file descriptors |
