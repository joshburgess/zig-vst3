...
| `FVariant`, `ICloneable`, `IPersistent`, `IAttributes`, `IAttributes2` | Initial struct and vtable translation complete; layout and IIDs checked against the pinned SDK with `zig build base-persistence-abi`; reusable persistent attribute store rejects oversized copied binary payloads without creating ghost attributes |
