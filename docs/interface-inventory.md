...
| `INoteExpressionController`, `IKeyswitchController`, `NoteExpressionTypeInfo`, `KeyswitchInfo` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build note-expression-abi`; bridge event conversion now rejects non-finite note-expression values |
