...
| `IPluginBase` | Initial vtable translation complete; component shell tests cover successful `IPluginBase` queries through the `IComponent` inheritance path |
| `IPluginFactory`, `IPluginFactory2`, `IPluginFactory3` | Initial vtable and struct translation complete; reusable static factory helpers in `factory.zig` cover metadata/class enumeration, fixed string truncation, invalid class clearing, create-instance dispatch, failed create output clearing, factory2/factory3 class metadata, Unicode class metadata, host-context storage, and query behavior; bundled example factories export the factory3 helper |
...
| `IStringResult`, `IString`, `IErrorContext` | Initial vtable translation complete; IIDs checked against the pinned SDK with `zig build base-strings-error-abi`; reusable string and error-context helpers cover bounded text storage, null string clearing, UI state tracking, missing-output rejection, and query behavior |
| `FVariant`, `ICloneable`, `IPersistent`, `IAttributes`, `IAttributes2` | Initial struct and vtable translation complete; layout and IIDs checked against the pinned SDK with `zig build base-persistence-abi`; reusable persistent attribute store covers queued values, copied and borrowed binary payloads, invalid enumeration, and oversized copied binary rejection without creating ghost attributes |
| `IUpdateHandler`, `IDependent`, `IPluginCompatibility` | Initial vtable translation complete; constants and IIDs checked against the pinned SDK with `zig build base-update-compatibility-abi`; reusable update-handler helper covers dependent registration, duplicate handling, deferred updates, invalid-dependent rejection, full-storage rejection without retaining, and query behavior; reusable compatibility JSON helper includes basic metadata fixture generation, deterministic streaming failures, and factory-level class exposure through `IPluginFactory3` |
| `IComponent`, `BusInfo`, `RoutingInfo` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build component-abi`; bundled component shells cover controller-class IDs, bus counts and bus info, invalid bus-info clearing, default routing behavior, validated bus activation, IO mode, activation state, host initialization, termination, state, and optional processor-side interface queries |
| `IAudioProcessor`, `IAudioPresentationLatency`, `IProcessContextRequirements`, and audio processor helpers | Initial vtable, struct, buffer, and iteration helper translation complete; layout checked against the pinned SDK with `zig build audio-processor-abi`; helper ranges reject negative sample counts, negative start offsets, and oversized byte counts; component and processing activation reject noncanonical boolean states without resetting processor state |
| Bypass processor constants and delay helpers | Initial helper translation complete; behavior checked against the pinned SDK with `zig build bypass-processor-abi`; helper ranges now reject invalid block sizes, circular-buffer positions, and oversized requested delay storage |
...
| `IEditController`, `IComponentHandler`, `IMidiMapping`, and adjacent edit-controller extensions | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build edit-controller-abi`; reusable component-handler helper added in `vst_component_handler.zig` with deterministic delegated system-time and progress failure outputs; automation parameter IDs, normalized values, and component restart flag masks are validated before state mutation, observer notification, recording, or delegation |
| `IProgress`, `IEditController2`, `IComponentHandler2`, `IComponentHandlerBusActivation`, `IEditControllerHostEditing`, `IComponentHandlerSystemTime` | Initial vtable translation complete; IIDs checked with `zig build edit-controller-abi`; reusable component-handler helpers cover automation callbacks, delegated failure propagation, editor/group callbacks, context-menu extension queries, bus activation, system time, progress callbacks, and unit-handler callbacks; dirty-state, bus-activation, progress-type, and progress-value inputs are validated before recording or delegation |
| `IParameterChanges`, `IParamValueQueue` | Initial vtable translation complete; IIDs checked with `zig build parameter-changes-abi`; reusable fixed-slot helper added in `vst_parameter_changes.zig`; signed point and queue counts clamp to storage capacity before indexing |
| `IEventList`, `Event`, event payload structs, and event helper functions | Initial vtable, struct, and helper translation complete; layout checked against the pinned SDK with `zig build events-abi`; reusable fixed-slot helper added in `vst_event_list.zig`; signed event counts clamp to storage capacity before indexing |
| `IConnectionPoint`, `IMessage`, `IAttributeList`, `IHostApplication`, and adjacent host/wrapper interfaces | Initial vtable translation complete; IIDs checked with `zig build host-message-abi`; reusable host-side helper added in `vst_host_application.zig` with deterministic failed and null-success factory outputs; reusable message and attribute helpers added in `vst_message.zig` with deterministic failed connection state and binary attribute storage |
| `IVst3ToVst2Wrapper`, `IVst3ToAUWrapper`, `IVst3ToAAXWrapper`, `IVst3WrapperMPESupport` | Initial vtable translation complete; IIDs checked with `zig build host-message-abi`; reusable wrapper marker and MPE support helpers added in `vst_host_application.zig`; wrapper marker tests cover VST2, AU, and AAX, and MPE settings use the SDK defaults and preserve previous accepted state on delegated failures |
| `IPlugView`, `IPlugFrame`, `IPlugViewContentScaleSupport`, and Linux run loop interfaces | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build plugview-abi`; reusable plug-view, plug-frame, and content-scale helpers preserve previous accepted state when delegated hooks reject a change, reject invalid scale factors and malformed resize rectangles before native or host delegation, validate delegated constraint output transactionally, and track host-visible input, focus, frame, size, attachment, removal, and resize behavior; reusable Linux run-loop helper covers event/timer callback delegation, query behavior, duplicate registration updates, null handler rejection, invalid file descriptor rejection, full-storage rejection, missing unregisters, and unregistered timer triggers; the standalone driver adds fixed-storage descriptor polling, bounded timer waits, missed-period coalescing, clock-regression containment, cross-target compilation, and exact retained-handler teardown |
| `IUnitInfo`, `IProgramListData`, `IUnitData`, `IUnitHandler`, `IUnitHandler2`, `UnitInfo`, `ProgramListInfo` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build units-abi`; reusable unit helper added in `vst_unit_data.zig` with deterministic delegated string and unit-by-bus failure outputs, fixed String128 truncation, program metadata and pitch-name callbacks, delegated unit/program data reads and writes, and signed count clamping before indexing |
| `IMidiLearn`, `IMidiMapping2`, `IMidiLearn2`, and MIDI controller mapping structs | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build midi-mapping-abi`; reusable MIDI learn and MIDI 2 mapping helpers cover assignment-list copying, configured bus direction, invalid output handling, and live controller tracking |
| MIDI controller constants | Initial constant translation complete; values checked against the pinned SDK with `zig build midi-controllers-abi`; MIDI output-value helpers now reject NaN normalized values before integer conversion |
| Speaker constants, expanded core speaker arrangements, `SpeakerArray`, and speaker helper functions | Initial constant and helper translation complete; values checked against the pinned SDK with `zig build speaker-core-abi`; speaker helper tests cover indexing, string lookup, named arrangement strings, 3D classification, ambisonic conversion rejection, stale speaker reset, and corrupted count clamping |
| Preset attribute keys, state type keys, musical instrument keys, musical style keys, and musical character keys | Initial string constant translation complete; values checked against the pinned SDK with `zig build preset-keys-abi`; representative taxonomy tests cover hierarchy separators, special characters, and character/state strings |
| Preset file chunk IDs, chunk type enum, and chunk ID helper functions | Initial constant and helper translation complete; values checked against the pinned SDK with `zig build preset-file-abi`; chunk helper tests cover every chunk type, the zero sentinel, case-sensitive ID comparison, and header/list offsets |
| `INoteExpressionController`, `IKeyswitchController`, `NoteExpressionTypeInfo`, `KeyswitchInfo` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build note-expression-abi`; reusable note-expression helper covers fixed expression and keyswitch metadata, invalid index clearing, delegated string conversion, and cross-interface query; bridge event conversion rejects non-finite note-expression values |
| `IPlugInterfaceSupport`, `IPrefetchableSupport`, `IAutomationState` | Initial vtable translation complete; constants and IIDs checked with `zig build capability-state-abi`; reusable interface-support and prefetch-state helpers added in `vst_capability_support.zig` with inflated-count clamping, delegated failure reset, and query coverage; reusable automation-state host helper added in `vst_host_context.zig` with delegated failure tracking |
| `IParameterFinder`, `IParameterFunctionName`, `IRemapParamID` | Initial vtable translation complete; constants and IIDs checked with `zig build parameter-helpers-abi`; reusable parameter helpers cover coordinate lookup, host function-name lookup, compatible-ID remapping, and deterministic missing-ID outputs |
| `IComponentHandler3`, `IContextMenu`, `IContextMenuTarget`, `IContextMenuItem` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build context-menu-abi`; reusable fixed-capacity context menu and target helpers cover balanced target retention through final menu release, failed lookup clearing, popup tracking, delegated item execution, query behavior, malformed item and group-flag rejection, and rejection of non-null `kNoParamId` targets before host delegation |
| `INoteExpressionPhysicalUIMapping`, `IInfoListener`, `PhysicalUIMap`, and channel context keys | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build physical-channel-abi`; reusable physical UI mapping helper added in `vst_capability_support.zig` with fixed-list, empty-list, delegated failure, and query coverage; reusable channel-context host helper preserves the last accepted context on delegated rejection |
| `IDataExchangeHandler`, `IDataExchangeReceiver`, `DataExchangeBlock` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build data-exchange-abi`; reusable data-exchange host helper added in `vst_host_context.zig` with deterministic failed and invalid-success factory, queue, block-lock, default lifecycle, and delegated block lifecycle outputs; lifecycle inputs reject zero capacities, invalid IDs, and noncanonical flags; receiver dispatch outputs are canonicalized; effect regressions cover queue-open, queue-close, valid block delivery, pointer/count mismatch rejection, and malformed block rejection |
...
| `IInterAppAudioHost`, `IInterAppAudioConnectionNotification`, `IInterAppAudioPresetManager` | Initial vtable translation complete; IIDs checked against the pinned SDK with `zig build inter-app-audio-abi`; reusable inter-app helpers added in `vst_inter_app_audio.zig` with deterministic failed screen-size and host-icon outputs, scheduled UI event delegation, remote-control tracking, preset-manager creation overrides, and connection notification callbacks |
| `ITestPlugProvider`, `ITestPlugProvider2` | Initial vtable translation complete; IIDs checked against the pinned SDK with `zig build test-plug-provider-abi`; reusable test provider helper covers provider2 factory, retained component/controller returns, paired `releasePlugIn` release, release tracking, and missing UID behavior |
