...
| `IDataExchangeHandler`, `IDataExchangeReceiver`, `DataExchangeBlock` | Initial vtable and struct translation complete; layout checked against the pinned SDK with `zig build data-exchange-abi`; reusable data-exchange host helper added in `vst_host_context.zig` |
