...
The Linux interoperability gate decodes independent FFmpeg, LAME, and Shine streams through the project and FFmpeg reference paths. Shine output without an optional Info frame matched 32,256 interleaved samples with `4.02e-7` peak error and `2.99e-7` normalized RMS error. A CRC-protected LAME stream matched 30,870 samples with every frame checksum validated, `3.73e-7` peak error, and `3.69e-7` normalized RMS error.
