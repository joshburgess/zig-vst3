...
`AdmStaticMatrixMixer` is the first bounded ADM rendering primitive. It binds one typed Matrix block to a caller-supplied, duplicate-free channel identifier order and retains only fixed input indexes and gains. Linear and dB coefficients render in f32 or f64 without allocation. Plan construction rejects missing inputs, duplicate terms, non-Matrix blocks, variable gains, phase, and delay. Block processing validates every buffer length and rejects overlapping input and output storage before mutation. Non-finite input or accumulation produces silence for that sample. `AdmDirectSpeakerRouter` binds a typed DirectSpeakers block to one unambiguous exact label in a caller-declared output layout. Its composable `mix` operation retains existing output content, applies the block gain, preflights shapes and aliasing, skips non-finite input samples, and contains non-finite output accumulation. Position-based DirectSpeakers selection, dynamic Matrix automation, delay and phase processing, Objects panning, HOA decoding, and binaural rendering remain separate layers.
