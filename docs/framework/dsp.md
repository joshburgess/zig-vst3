...
`AudioMetadataEntry` represents a borrowed FourCC and byte string. `encodeRiffInfoMetadata` and `encodeAiffTextMetadata` write complete metadata chunks into caller storage, while their iterators validate and expose both known and unknown FourCC entries without allocation. `RiffInfoMetadataView`, `AiffTextMetadataIterator`, and `RiffXmlView` apply the 256 MiB `default_audio_metadata_limits` encoded-byte policy. Their `initWithLimits` constructors accept a caller-selected `AudioMetadataLimits` policy and retain it for hostile-state validation. The encoded output prefix must not overlap the entry descriptors or any borrowed value. Alias rejection leaves the destination unchanged. Both iterator kinds expose `valid` and bind ordinary traversal to exact source-and-cursor witnesses. A missing or inconsistent witness falls back to canonical prefix reconstruction and accepts only exact entry boundaries. Rejection does not change the cursor. `WavFileWriter`, `AiffFileWriter`, and `Rf64FileWriter` accept these tags through `initWithMetadata`. Metadata values need to remain valid only for that initialization call.
...
`AdmXmlDocument` adds an allocation-free typed graph over a bounded XML scanner. `AdmXmlDocument.init` applies `default_adm_xml_limits`, while `initWithLimits` accepts an explicit `AdmXmlLimits` policy. The defaults admit 16 MiB and one million XML events, then independently bound declarations, references, profiles, tag groups, tags, targets, blocks, extensions, typed and untyped attributes, and graph-validation work. Custom policies may tighten any bound, while the fixed declaration index supports at most 4,096 local declarations. The document retains the policy with its borrowed input, so later reference and profile-uniqueness checks use the same work ceiling.

The graph recognizes all core declaration families inside one `audioFormatExtended` subtree and exposes declaration and reference iterators. Construction uses exact fixed-storage indexes for duplicate declarations, reference resolution, cardinality, reciprocal stream and track links, matrix coefficient targets, and per-channel block sequences. Validation rejects wrong reference kinds, unresolved custom definitions, invalid owners, self-references, incompatible type labels, broken reciprocity, invalid identifier continuity, and excess or mutually exclusive references. Common format definitions may resolve outside the document.

The XML scanner resolves scoped default and prefixed namespace declarations, validates namespace URI references through 2,048 decoded bytes and reserved bindings, compares entity-normalized namespace names, and rejects undeclared prefixes or duplicate expanded attributes. An element accepts at most 1,024 attributes and 256 KiB of attribute source, bounding the pairwise duplicate and expanded-name checks. Attribute iteration validates its complete retained source once, then binds every successful offset to an exact witness. Event iteration binds the offset, open-element stack, namespaces, and borrowed ranges to an exact witness. Ordinary traversal therefore advances without canonical prefix replay. A missing or inconsistent witness falls back to complete canonical reconstruction, preserving rejection of hostile retained state. Attribute and event iteration commit cursor, witness, and nested state only after a complete parse step succeeds. Ordinary escaped text and CDATA sections share one bounded typed-text path. CDATA remains literal while entity references in ordinary text decode normally, including when the two forms are adjacent. DTD declarations remain explicitly unsupported. The typed graph matches elements against the expanded namespace of `audioFormatExtended`. It omits complete foreign-vocabulary subtrees, so nested elements cannot impersonate metadata fields. Attributes are matched by exact qualified name, so a foreign namespace cannot satisfy a required unqualified field.
...
Stream parsing requires a stable version, sample rate, channel count, and free-format mode, skips one bounded leading ID3v2.2, v2.3, or v2.4 tag and one trailing ID3v1 record, and rejects truncated frames or trailing data. `Mp3Stream.init` and `Mp3FileReader.init` apply `default_mp3_limits`, which accepts at most 4,294,967,295 encoded bytes and 10,000,000 frames. Their `initWithLimits` and `summarizeWithLimits` variants accept a caller-selected `Mp3Limits` policy. Byte limits are checked before scanning, and frame-limit rejection occurs before cursor, counter, or destination mutation. ID3 extent checks use ordered subtraction rather than potentially overflowing end-offset addition. Cursor, first-header, free-format base length, frame-count, and sample-count changes commit together only after all derived values validate. The reader exposes `valid` for retained boundaries, header fields, frame-to-sample counter coherence, free-format base state, selected limits, and enough consumed bytes to contain every completed frame at the format's smallest compatible Layer III frame size. The same validation runs before iteration and recovery. The tables and ordinary bitrate frame formulas follow FFmpeg's maintained [MPEG Audio header decoder](https://github.com/FFmpeg/FFmpeg/blob/master/libavcodec/mpegaudiodecheader.c).
...
`decodeInterleavedFlac` validates metadata and complete frame boundaries before returning caller-owned interleaved `i32` samples. Memory decoders and `FlacFileReader` apply `default_flac_limits`: one TiB of encoded data, 1,000,000 metadata blocks, 256 MiB of complete metadata blocks, 10,000,000,000 PCM frames, and 10,000,000 decoded FLAC frame blocks per operation. The `WithLimits` memory entry points and file-reader constructors accept a caller-selected `FlacLimits` policy. File metadata preflight applies the same stream, metadata, and declared-frame policy before retained metadata storage changes. Accepted file readers retain their exact metadata totals and selected policy as part of `valid`. Before decoding, the parser relates the encoded audio extent to the frame-count interval implied by STREAMINFO block and frame-size bounds. Too-short input retains `TruncatedFlac`; an impossible oversized declared extent returns `InvalidFlacFrameSize` before output mutation. It decodes constant, verbatim, fixed-predictor, and LPC subframes, both Rice methods, escaped residual partitions, wasted bits, and left-side, side-right, or mid-side stereo. Invalid fixed-predictor orders return `InvalidFlacPredictorOrder` in both encoding and decoding. The ordinary direct decoders may retain samples written before a later frame, digest, or frame-block limit error. `decodeInterleavedFlacTransactional` stages the complete result in caller scratch and leaves the destination unchanged on every failure. Direct and file-backed decode APIs reject overlap among encoded input, decoded output, decoded-frame scratch, and wide side-channel scratch before parsing, positional reads, or output mutation. The bounded convenience functions `writeInterleavedFlacFile` and `readInterleavedFlacFile` use caller storage for the complete encoded file. Complete and range transactional read variants provide the same rollback contract, and wide-scratch variants cover 33-bit stereo side channels. The dedicated fuzz target combines arbitrary input with generated FLAC streams, byte mutation, truncation, extension, explicit limits, and transactional decoding. These are offline file utilities, not audio-callback streaming primitives.
...
The independent decoder gates pin separately authored stb_vorbis 1.22 and Xiph Tremor by exact revisions and source hashes. Fifteen project-, FFmpeg-, Xiph-, and fixed-point-specific cases compare complete floating-point and signed-16 output across mono, stereo, 5.1, low rates, low quality, low-level and clipping boundaries, both standard block families, mixed transitions, multi-page packets, exact terminal samples, and compatible chains decoded explicitly per logical link. Strict Ogg preflight and transactional publication reject checksum damage, header and audio truncation, invalid audio packet types, incompatible chain geometry, and an injected failure in a later logical link. Additional implementation families and audition remain open.
...
The standalone `BoundedCaptureRateBridge` composes one stream per channel with a bounded SPSC FIFO and `ClockDriftController`. Products select FIFO target, ppm and slew limits, controller gains, startup and recovery fill thresholds, correction cadence, and underflow and overflow policy. A fixed nonzero correction cadence makes results independent of native callback partitioning while preserving phase across ordinary drift. Priming and policy-selected recovery emit deterministic silence without consuming a new timeline.
