...
- Native assistive-technology bridges. macOS is integration-tested, Windows is cross-compiled, and Linux has tested semantic mapping plus application-root, Accessible, Component, Action, Value, X11 clipboard-backed EditableText, bulk Cache publication, cache lifecycle signals, and property, state, focus, and bounds events. Pure-Wayland clipboard ownership and native screen-reader workflows remain unverified.
