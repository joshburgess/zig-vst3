...
Declare up to two `PresetBrowser` values in `EditorDescription.preset_browsers`. Each browser accepts up to 64 stable, nonzero preset IDs. Search and selection reference declared persistent editor-state fields. The search field must use `editor_state.Text`; the selection field must use `index` or `point_id`. Restoration validates both fields before changing the browser, falls back to the first matching preset when a saved ID is unavailable, and resets transient load status only after a successful restore.
...
Range-selection configurations reject non-finite derived spans as well as non-finite fields. Viewport projection returns a neutral coordinate when finite retained state and input would derive an infinite result.
...
`gui_file_importer.Snapshot.validate` rejects impossible path counts, progress, and cancellation states. Its progress and action queries return neutral values unless the complete snapshot is valid. `gui_audio_file_importer.Snapshot.validate` also enforces the shared preview, frame, channel, sample-rate, and decoded-frame limits. Preview and decoded-sample copies validate retained bounds and finite values before changing caller output. The native bridge validates every snapshot returned by a plugin controller before narrowing fields or exposing it to the adapter. Unknown picker and import-command values are rejected before plugin callbacks. Controller-sourced graph callbacks receive at most 256 points, and oversized results are clamped to that capacity.
...
Implement `loadGuiProgress` and return a `ProgressSnapshot`. Determinate values must be finite and in `[0, 1]`; idle snapshots use zero, complete snapshots use one, and indeterminate running snapshots do not carry a determinate value. Generation zero is reserved for the initial idle state. The generation identifies logical work and lets consumers distinguish replacement from continued progress.
...
- Native assistive-technology bridges. macOS is integration-tested, Windows is cross-compiled, and Linux has tested semantic mapping plus application-root, Accessible, Component, Action, Value, Unicode-segmented single-line Text queries, Unicode-delta text-change events, X11 and Wayland clipboard-backed EditableText, bulk Cache publication, cache lifecycle signals, and property, state, focus, and bounds events. Built-in visual caret and selection integration, live Wayland clipboard behavior, and native screen-reader workflows remain unverified.
