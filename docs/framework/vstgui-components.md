...
`RetainedSource.loadText` copies a processor-provided status or metadata string into caller-owned bounded storage. The processor returns the number of bytes copied and must not retain the output slice. Truncation is explicit when the returned count equals the supplied capacity. Text loading is for low-rate editor snapshots such as a model name, resource status, or format summary. It must not be called from processing or used as a streaming log channel.
