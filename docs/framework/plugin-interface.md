...
Call `try Spec.initChecked(.{})` when parameter descriptors come from runtime input. `Spec.init` accepts only compile-time descriptors and turns invalid fixed metadata into a compiler error.
...
Plugin control code can use `HostRequestSink.setAudioBusLayout`, `addAuxiliaryAudioBus`, or `removeAuxiliaryAudioBus`. These mutations are non-realtime control-thread operations. Debug and test realtime scopes reject them before component locking. Successful effective changes publish one snapshot and mark `kIoChanged`; call `dispatchPending` outside processing to notify the host. The raw `SimpleEffect` type exposes matching operations plus `audioBusTopologySnapshot` and `dispatchHostRequests` for integrations that own the component interface directly. Those raw operations have the same non-realtime restriction and audit guards. Host-initiated arrangement and activation callbacks update the published topology without sending a redundant restart.
...
`ProcessorRuntime` enforces `initialized -> prepared -> active` transitions. Processing requires an active runtime and a context whose sample rate, mode, and block size match the preparation contract. Deactivation calls `deactivate` and `reset`. Resource release returns the runtime to its initialized state. Teardown performs any remaining deactivation, reset, and release before entering the `deinitialized` terminal state and calling the product hook. Repeated teardown is a no-op. Fallible operations report `ProcessorDeinitialized` afterward, while latency and tail queries return zero without reading released product state. `initInto` may construct a new runtime in the same storage.

Direct `PluginInstance.prepare` calls validate the sample rate and maximum block size and return an error for invalid configuration. `prepareChecked` remains an equivalent compatibility spelling. Neither entry point invokes the plugin's preparation hook until validation succeeds.
...
Generated LV2 metadata groups the flattened audio ports back into their declared streams. Main input and output resources use `pg:mainInput` and `pg:mainOutput`. Every auxiliary input is an input group with `pg:sideChainOf` when a main input exists, and the main output identifies the main input as its source. Mono, stereo, common surround, and first- through third-order ACN layouts use the matching standard port-group classes and channel designations. Other discrete, immersive, and fourth- through seventh-order ACN layouts retain generic discrete or ambisonic group classes where the vocabulary has no exact class or designation. Group symbols are generated in the same namespace as port symbols, and parameter metadata that collides with them is rejected before output. Plugin, project, UI, preset, port-group, and state-key identities must also remain distinct. Exact collisions are rejected before the writer receives any bytes.

Atom event ports use frame timestamps and a fixed 256-event stack bound per run. MIDI note on, note off, polyphonic key pressure, control change, and pitch bend translate to and from typed framework events. Other complete valid MIDI messages, including Program Change, Channel Pressure, System Common, System Real-Time, and SysEx, cross the processor boundary as data events tagged with the host's MIDI event URID. Other nonzero Atom types retain their URID and bounded body bytes in data events. Plugins may inspect, transform, or echo those bytes without allocation. Malformed sequence boundaries, timestamps, types, or MIDI payloads reject the run transactionally. Output serialization observes the capacity supplied by the host and preserves 64-bit Atom offsets. Boundary reads and writes remain safe even if a hostile host supplies an Atom port base with only byte alignment.

Event-capable plugins accept `bufsz:sequenceSize` through instantiation Options and the runtime Options interface. `configuredSequenceSize` returns the latest host value or null when the host has not supplied one. Runtime updates are valid while processing is active because the value is a configuration hint and does not resize adapter storage. The connected Atom buffer remains authoritative for every run.

Plugins that declare `bindLv2PortResize` receive an instance-owned `PortResizeSink`. During processing, `resizeOutput` forwards checked requests to the optional host `rsz:resize` feature. The sink rejects zero-byte requests, non-output ports, indexes outside the C ABI, calls outside processing, and calls when the host omitted the feature. An advertised feature with a null callback rejects construction. Generated metadata advertises `rsz:resize` as optional for declarations that bind the sink. Unrecognized host status codes normalize to `unknown`. The plugin must handle every non-success result without assuming that the connected buffer grew.

Audio and control ports require natural f32 alignment before they enter the format-neutral processor. Misaligned audio storage rejects the run as an invalid context, while misaligned parameter or freewheeling storage rejects it as invalid control input. Failure cleanup writes silence through byte-aligned output access, so rejecting a malformed input cannot trap while clearing another malformed output. Latency publication also supports byte-aligned host storage.
...
Plugins that inspect host-defined Atom types may opt into URI reverse mapping by declaring `lv2_urid_unmap_required = true` and implementing `bindLv2UridUnmap`. The binding receives an adapter-owned `UridUnmapSink` for the plugin instance lifetime. Its `unmap` method returns a borrowed byte slice for a known identifier or null when the identifier is unknown. Host results must be nonempty and terminate within `maximum_unmapped_uri_bytes`, currently 4,096 bytes. Invalid results return null instead of requiring plugin code to perform an unbounded scan. The adapter rejects construction when the required feature is absent, null, misaligned, or has a null callback, and generated metadata adds `urid:unmap` to the required features. Plugins that do not declare the opt-in retain the existing construction contract.

The adapter exposes the LV2 State interface through `extension_data`. When the host supplies the standard URID map feature, parameter state is stored under the plugin URI plus `#parameterState` as a portable POD Atom Chunk. A plugin that declares `component_state_maximum_encoded_size`, `writeComponentState`, and `readComponentState` receives a second property under `#componentState`. Component state is limited to 64 KiB and must encode at least one byte. Resource-backed plugins may add `writeLv2ComponentState` and `readLv2ComponentState`. These hooks receive `StatePathFeatures`, which maps absolute paths to portable abstract paths, resolves abstract paths during restore, creates host-managed paths when `lv2_state_requires_make_path` is true, and returns `OwnedStatePath` values that must be released with `deinit`. Release clears the retained host pointer and length, so later `bytes` calls return an empty slice and repeated `deinit` calls do not release the host allocation again. Input and host-returned paths must be nonempty and at most `maximum_state_path_bytes` bytes. Host-returned values must terminate within that 4,096-byte bound and are released exactly once on rejection. Accepted values retain their validated length, so `OwnedStatePath.bytes` does not rescan host memory. Generated metadata requires `state:mapPath` and `state:freePath`, plus `state:makePath` when declared. Each state path feature must occur exactly once, and thread-safe restore requires exactly one Worker schedule feature. The adapter rejects duplicate, missing, or misaligned required features and null path callbacks before plugin state access. It also rejects a null host store or retrieve callback before state work. `StatePathFeatures` retains only validated non-null callbacks for the duration of the state operation. Restore requires both properties to retain the portable POD flags written by the adapter, validates them before mutation, rejects empty component payloads and unread trailing bytes, and restores the previous parameter snapshot when component or path decoding fails. A successful component restore invokes `afterComponentStateRestore` when present. Missing parameter state resets parameters to defaults; missing component state leaves the current component payload unchanged.

Plugins whose internal state can change without a host request may bind the standard StateChanged notification sink:

```zig
pub fn bindLv2StateChanged(
    self: *@This(),
    state_changed: *plug.lv2.StateChangedSink,
) void {
    self.state_changed = state_changed;
}
```

The sink remains valid for the plugin instance lifetime and `notify` is allocation-free and safe to call from any thread. Notifications coalesce by generation and produce one `state:StateChanged` Object on the next successful non-empty audio block. The event is placed at the final frame so it remains ordered after other output events. A disconnected or undersized event output does not consume the pending generation, and a concurrent notification after serialization remains pending for the following block. Generated metadata advertises the Object only on the event output port. Plugins should not notify for changes initiated by a host message that already tells the host state may have changed.

Declare `bindLv2Log` to receive an instance-owned `LogSink`. `writeNonRealtime` accepts error, warning, and note levels and must stay outside realtime processing. `trace` is the only logging call intended for the realtime path. Both methods return null when the host did not provide logging or the message contains an interior NUL. Plugin messages are sent through a fixed string format, so percent characters remain data rather than host format directives. Duplicate features, null or misaligned feature data, and null write callbacks reject construction. Generated metadata advertises `log:log` as optional.

Core and UI construction require a non-null host feature-array pointer, as required by their ABIs. A host with no features passes a one-element array containing null. Construction rejects arrays that are misaligned, lack a null terminator within the 256-entry scan bound, contain a misaligned feature record, or contain a record without a URI. Known feature URIs use exact comparison bounded by the supported URI length plus one byte. Core extension-data, UI extension-data, and UI plugin-URI matching use the same bounded comparison. Construction also rejects duplicate URID map, Options, Worker schedule, resize-port, logging, or required URI reverse-map records before allocating an instance. Required reverse mapping must appear exactly once. Optional features may be absent, but an ambiguous duplicate cannot select one of several host callbacks by array order. State feature scans apply the same structural validation before using a supplied array, while State calls may omit their additional-feature array.

Declare `lv2_thread_safe_restore = true` to support state restoration while audio processing continues. When the restore call includes a valid Worker schedule feature, the adapter validates and copies parameter and component state into fixed instance storage without mutating the plugin. One reserved empty Worker request produces an empty response, and the response callback applies the staged state in the audio threading class. A second restore returns `no_space` until the first response succeeds or fails. Successful application and every rejected staging, scheduling, response, or component-application path scrub the fixed staging storage before returning the slot to idle. Missing Worker scheduling retains the ordinary synchronous restore path. Products that also use the Worker interface must use nonempty request and response payloads when this option is enabled.

Path-aware component state needs a control-thread conversion because host path callbacks are valid only during restore. Declare `lv2_thread_safe_restore_maximum_component_size`, static `stageLv2ThreadSafeComponentRestore(reader, paths, writer)`, and instance `applyLv2ThreadSafeComponentRestore(reader)`. The staging hook resolves portable paths and writes a bounded application payload without accessing plugin state. The apply hook decodes that payload in the audio threading class. Full consumption, nonempty output, path failures, decode failures, parameter rollback, and pending-restore exclusion are enforced by the adapter. Generated metadata publishes `state:threadSafeRestore` as an optional feature and the Worker extension interface.
...
pub fn readLv2PatchGraph(
    self: *const @This(),
    request: plug.lv2.PatchGraphGetRequest,
) !plug.lv2.PatchAtomValue {
    return self.describeResource(
        request.subject,
        request.accept,
        request.context,
    );
...
Put, Insert, Patch, and Move require one subject. Delete and Copy accept one through `maximum_patch_graph_subject_count` subjects. Body, add, and remove values preserve the host Atom type and borrow their body storage only during the callback. Context, sequence number, and request identity are optional. Graph replies preserve the context as a URID Atom. A nonzero sequence number or non-anonymous request identity produces a correlated `patch:Ack` after success or `patch:Error` when the callback returns an error. An explicit sequence number zero suppresses the response. The request identity retains its Object or Blank Atom type, object ID, and message type for the current run.

A property-less `patch:Get` invokes `readLv2PatchGraph` when graph queries are enabled. The callback receives the optional subject, accepted response class, context, sequence number, and request identity. It returns one opaque Atom body, which the adapter copies into a bounded `patch:Put` response before the callback returns. Sequence number zero suppresses this response. A callback error produces a correlated `patch:Error` when the request supplies a nonzero sequence number or request identity.

Property-specific Get responses preserve the same request identity in their bounded Set response. Anonymous Gets without a sequence number continue to receive a simple Set or Put response. Request identities are scoped to one run and must not be retained as globally meaningful identifiers.

The adapter validates the complete object and applies graph requests at their event frame. Recognized malformed requests reject the block before a graph callback runs. A disagreement between the parsed Patch kind and the selected dispatch path returns `InvalidPatch` instead of terminating the process. Graph support is independent of typed property declarations, and graph-only metadata still marks both event ports as supporting `patch:Message`.
...
`plug.lv2.metadata.Generator` writes the manifest, plugin description, and standard Presets vocabulary data from the same plugin, adapter, URI, and initial parameter declaration used by the binary. It derives port indexes, symbols, names, ranges, defaults, event capabilities, Options, State, optional thread-safe restore and Worker declarations, and the Programs interface when program lists exist. Factory presets identify values by parameter field symbol. Optional `ProjectMetadata` writes a linked project name, license URI, and maintainer name, `mailto:` URI, and homepage. The top-level metadata can also declare long and short descriptions and live-use intent. Optional `UiMetadata` associates the plugin with a separate UI URI, class, and binary and declares parent, touch, idle, show, Options, port-map, port-subscription, peak-protocol, and Programs UI capabilities. Its `port_notifications` can request static host delivery by stable symbol with `.float` for control ports, `.peak` for audio ports, or `.event` for an event output plus a required Atom type URI. Several event notifications may share one output when their type URIs differ. Set `event_input_messages` only when the editor sends typed messages to a generated event input. Either typed direction advertises the standard Atom event-transfer protocol. Generation rejects an outbound declaration without an event input plus malformed, unknown, protocol-incompatible, or exact duplicate notifications before writing output. Event notifications also emit notify-type metadata. Its `resize_behavior` is `.resizable` by default. Use `.programmatic_only` when the host must not offer user resizing but the UI may request size changes, or `.fixed` when the UI never changes size. Fixed metadata advertises `ui:fixedSize` and omits both resize feature and extension-data claims. Programmatic-only metadata advertises `ui:noUserResize` while retaining both resize paths.

Generation rejects malformed URIs, maintainer email URIs, filenames, text, symbols, duplicate preset slugs or parameters, unknown parameters, non-finite or out-of-range values, and exact identity collisions among generated resources. URI schemes must begin with an ASCII letter and may continue only with ASCII letters, digits, plus, hyphen, or period. Percent escapes require two hexadecimal digits, and Turtle IRI references reject their forbidden ASCII delimiters. State keys, presets, and port groups derived from a fragment-bearing plugin URI extend its existing fragment with a slash. Turtle text rejects unsafe C0 controls, DEL, and the C1 control range while retaining tab, line feed, carriage return, and ordinary Unicode text. The Mono Gain build generates Unity and Muted metadata plus its project and distribution metadata into a transactional staged bundle, so hosts can discover them without loading the shared library. The generated Turtle passes the LV2 1.18.10 schema validator with zero errors and passes `lv2lint` 0.16.2 with warnings treated as errors in direct-distribution mode. `zig build lint-lv2-mono-gain` reproduces both checks when the external tools are installed. `LV2_VALIDATE` and `LV2LINT` select explicit executables.

The optional LV2 Options feature may provide instance-level minimum, maximum, and nominal block lengths as Atom Int values during construction. The maximum narrows the adapter's compile-time capacity and the runtime preparation contract. Construction rejects a null, misaligned, bounded-but-unterminated, or structurally malformed feature-pointer array. A null or misaligned advertised feature value, zero or negative maximum, negative minimum or nominal value, oversized maximum, repeated or mistyped value, unterminated option array, or internally inconsistent set rejects instantiation without publishing a partial instance. Runtime queries validate the complete bounded array before publishing size, type, or value fields, so a later malformed entry or missing terminator leaves every query unchanged. The same checked decoding applies to the advertised URID map feature before any callback is invoked. An advertised URID map with a null callback rejects construction.

The Options interface returns stable Atom Int values for all three supported bounds. `set` validates the complete request before changing the instance. A null option array returns the unknown status. A maximum change while active is rejected. While inactive, a valid maximum change reprepares the processor; the published bounds change only after preparation succeeds, and a preparation failure returns the unknown status after attempting to restore the previous preparation. Unknown keys, unsupported subjects, repeated keys, invalid Atom values, and unterminated arrays return the standard composable Options status bits.

`plug.lv2.ui.Adapter` exports one LV2 UI descriptor over the shared `gui.Editor` contract. A backend constructs the editor and returns its native child widget after parent attachment. The bridge converts plain f32 control-port events to normalized editor values and normalized editor gestures back to host writes. It supports optional host touch, resize, `ui:requestValue`, port map, port subscription, and peak protocol features plus idle, host resize, show, hide, Options, and Programs UI interfaces. `Context.requestHostValue` maps its validated key and optional type URI through the retained host URID map, calls `ui:requestValue` with no request-specific features, and translates success, busy, unknown, and unsupported statuses. An unmapped key reports unknown, an unmapped requested type reports unsupported, and an unrecognized host status fails closed as unsupported. The callback is optional. Missing, null, misaligned, or duplicate records disable it without rejecting the UI.

`Context.subscribeHostPeak` maps a validated stable port symbol to its host port index and records the editor's source ID. The default `.dynamic` delivery calls the standard port-subscription feature and records the source only after host acceptance. `.static` delivery records an update already declared through `UiMetadata.port_notifications`, so it requires port mapping and the peak protocol but not the dynamic subscription callback. The adapter retains at most 16 unique peak sources. It copies each exact `PeakData` event from borrowed host storage, rejects malformed sizes, periods, values, formats, and unregistered ports, then delivers a format-neutral `HostPeakMeasurement` to the backend. Explicit removal of a dynamic source changes state only after host success. Static removal never invokes that callback. Cleanup and every partial-construction rollback unsubscribe dynamic records before destroying the editor and discard static records locally. Missing, null-callback, misaligned, or duplicate features leave the affected delivery mode unavailable without rejecting the UI.

`Context.registerHostAtomNotification` maps a validated event-output symbol and Atom type URI to a unique source ID. The UI bridge enables this path only when the host supplies port mapping, URI mapping, and the standard Atom event-transfer protocol. It retains at most 16 static bindings. `port_event` accepts one complete Atom header and body with an exact declared size, rejects type zero, unknown ports or types, trailing bytes, truncation, and bodies above 64 KiB, then exposes only the borrowed body through `HostAtomMessage`. Cleanup discards these static bindings without a host unsubscription call.

`Context.sendPluginAtomMessage` maps a validated event-input symbol and Atom type URI, serializes one complete Atom into instance-owned 64-bit-aligned scratch storage, and calls the host UI write function with the standard event-transfer protocol. It rejects missing features, unknown symbols or types, and bodies above 64 KiB before calling the host. The scratch packet is erased immediately after the host callback returns, so the host must consume or copy it during that callback. The call has no timestamp because the protocol transfers one individual event rather than an event-sequence record.

The native `vstgui_lv2_backend.Description` can declare `peak_sources` and VSTGUI `meters`. Each source pairs one stable port symbol with one source ID and selects dynamic delivery by default or static metadata delivery explicitly. Backend construction registers every source, stores accepted measurements in instance-owned slots, and serves meter polling by source ID. Missing host support leaves the meters at zero without preventing the editor from opening. Compile-time validation limits the description to 16 unique sources and eight meters, rejects duplicate symbols or source IDs, and requires every meter channel to name a declared source. The Mono Gain LV2 UI demonstrates statically declared input and output peak meters.

When the host supplies Options and URID map features, the adapter accepts `ui:scaleFactor` and `ui:updateRate` Atom Float values, a `ui:windowTitle` Atom String, and `ui:backgroundColor` and `ui:foregroundColor` Atom Int values in RGBA32 form. Scale applies uniformly after attachment and before widget publication. Update rate defaults to 60 Hz, the window title defaults to the plugin name, and colors default to opaque black and opaque white. Optional backend `updateRate`, `windowTitle`, `backgroundColor`, and `foregroundColor` hooks receive the effective values on the UI thread. The runtime Options interface reports all five values and applies validated changes. It owns up to 255 UTF-8 title bytes and copies both color values instead of retaining host storage. Queries validate the complete bounded request before publishing any result, so mixed malformed or unterminated arrays remain unchanged. Runtime updates validate every key, type, size, finite positive numeric value, UTF-8 byte, terminator, and duplicate before mutation. A null URID map callback rejects construction before mapping, while a null or malformed option array leaves current values unchanged. A backend scale rejection preserves the last accepted values. Invalid construction values reject construction and tear down the partial editor. Program selection updates affected editor parameters without echoing writes or gestures to the host. Instantiation validates non-null descriptor, plugin URI, and bundle path pointers plus the plugin URI value, required parent feature, write callback, widget output, backend construction, attachment, options, and widget creation before publishing a handle. Feature scanning rejects a null, misaligned, unterminated, or structurally malformed array before backend construction. Duplicate parent, URID map, or Options feature records reject construction. Duplicate optional touch, resize, or request-value records disable that optional callback. A null extension-data URI returns no interface. Null, misaligned, or null-callback optional callback data is ignored. Valid optional callbacks are copied into checked instance-owned views. Descriptor callbacks and editor callbacks share the same checked handle conversion, so a misaligned handle cannot reach editor state through either path. Malformed port events and resize requests are ignored or rejected without partially updating the editor.
...
`vst3.ara_source_cache.Cache(ControllerType, Sample, limits)` bridges the non-realtime ARA reader API to realtime playback. `loadSource` opens the requested f32 or f64 host reader on a host-authorized model or control callback, fills an inactive fixed-capacity generation in place, closes the reader, and publishes only after the complete read succeeds. A failed open, read, capacity check, or publication leaves the previous generation active. `invalidate` publishes an unavailable generation, while generational source identity prevents a recycled model slot from exposing stale audio. The returned renderer provider performs no allocation, locking, I/O, or waiting. It acquires a bounded reference, validates source identity, format, retained dimensions, and disjoint destination channels before indexing, copies the requested planar range, and releases the reference. A malformed retained generation or overlapping destination reports an error without changing the destination. Cache memory is fully inline and scales with sources, channels, frames per source, and publication slots, so products must choose limits deliberately. Keep the cache at a stable address from provider creation until every renderer using that provider has detached.

`vst3.ara_source_cache.PagedCache(ControllerType, Sample, limits)` provides the same renderer-provider contract for sources larger than a practical whole-source generation. Its fixed storage is divided into independently reference-counted pages and one reference-counted directory. `loadRegion` computes the source-frame working set for a render description, including interpolation padding. `loadRange` merges a caller-selected range, retaining matching pages and replacing the least-recently-used unprotected pages. `refreshRegion` and `refreshRange` force replacement after source-content changes. A resident refreshed page keeps its directory slot, preventing a stale duplicate entry from shadowing the new generation. All required page writers and the next directory writer are reserved before host I/O. The cache fills every page, commits their immutable generations, and publishes the directory last. Failed validation, reservation, open, or read leaves the old directory active. Realtime reads pin one directory, then pin and validate every required page against its recorded generation, source identity, logical page, channel count, sample rate, fixed storage bounds, and exact logical extent before copying any samples. A host frame position that cannot fit the target address width fails with `InvalidAudioBuffer`. A malformed retained page, publication race, or missing later page produces a checked cache miss without changing the destination. Explicit invalidation removes every matching directory entry without touching readers. Page count, page size, channel count, source slots, and publication depth are compile-time bounds.

`vst3.ara_spectral_transform.PreparedSource` publishes fixed-capacity transformed source generations for realtime rendering and can fall back to another source provider when no matching transformed generation is available. Realtime lookup validates the retained sample rate, channel count, and frame count against fixed storage before indexing. Reads validate every destination length and reject overlapping channel spans before copying the first channel, so malformed retained state and bad output layouts leave all destinations unchanged. The fallback path ignores malformed prepared generations. Inactive generations and invalidation use an empty source identity, so no unpublished slot retains indeterminate identity bytes.

`vst3.ara_content_fades.Analyzer(ControllerType, Sample, limits)` derives content-based head and tail fade descriptors without audio-thread host access. On the model thread it reads bounded source windows, validates every sample, selects the channel with the greatest AC energy, and searches for a zero crossing after the configured minimum fade. A bounded fallback applies when no crossing is present. Durations are converted through the region's modification-to-playback ratio and proportionally reduced when two requested fades would overlap beyond the region duration. The fixed-capacity analyzer publishes only for the exact source, region, transformation, format, and timing description it analyzed. Failed analysis leaves the prior descriptor intact, while source invalidation replaces every matching entry with an empty description and fade. `configureProvider` attaches this state to a renderer provider without changing its realtime sample source.

`Controller.copyHostMusicalContextContent` and `copyHostAudioSourceContent` copy typed ARA content from optional host readers into caller-bounded storage. They validate availability, ranges, event counts, event pointers, and grades, and always destroy a successfully created host reader on success or failure. `vst3.ara_tempo_warp.Builder(ControllerType, limits)` uses those methods for host-provided tempo entries or accepts product-provided modification and context maps directly. Maps require finite, strictly increasing time and quarter positions, at least two sync points, and an explicit quarter-zero point. The builder merges every relevant source and context tempo boundary, verifies that mapped endpoints agree with the playback region, and publishes a bounded, strictly increasing piecewise warp only for the exact model revision and render description. Failed preparation retains the prior plan. Replacing a plan clears its unused point capacity. Source or global invalidation replaces stale plans with empty descriptions and points.

`vst3.ara_playback_renderer.Renderer(ControllerType, Sample, limits)` is a reusable playback-role implementation for f32 or f64 products. It observes assignment and graph publications, resolves generational region references, and publishes an immutable render plan through `RealtimeSnapshotPublisher`. Every inactive plan region starts with an empty render description and tempo-warp capacity. Plan publication validates finite source and time fields, finite derived endpoints, and finite source-frame products before realtime state changes. Construction requires a `SourceProvider`. Its control-thread `ready` callback confirms that product-owned cached or analyzed audio matches a render description, while its realtime `read` callback supplies samples without allocation, locks, I/O, or blocking. Optional model-thread callbacks supply exact fade descriptors and copy tempo-warp points into the immutable plan. `refreshSourceState` republishes the plan after cache readiness changes. The renderer never calls ARA's potentially blocking host audio or content readers from the audio thread. Rendering requires equal-length, disjoint output channels and clears supplied storage after any rejected shape or render failure. It uses fixed planar scratch storage, binary-searched piecewise tempo mapping or ordinary playback-to-modification mapping, selectable linear, Catmull-Rom cubic, or eight-tap windowed-sinc source interpolation, linear, smoothstep, or equal-power content fades, overlap summing, and deterministic failure silence. Cubic mode reserves one preceding and two following source samples. Windowed-sinc mode reserves three preceding and four following samples, selects normalized Lanczos weights from a compile-time 1,024-phase table, preserves constant signals and exact source samples, and duplicates the nearest endpoint where the source has no neighbor. Its realtime path performs one phase lookup and eight multiply-adds per rendered sample and channel, without interpolation trigonometry. Its analytic high-frequency test verifies at least a tenfold RMS error reduction relative to linear interpolation. The current renderer accepts ordinary playback, duration-ratio time stretching, tempo-reflecting time stretching, and content-based head and tail fades. Processing scratch remains transient and is written before use. A product that needs phase-vocoder or other spectral editing should supply its own renderer over the same controller and extension contracts.
...
`BoundedCaptureFifo(Sample, maximum_channels, frame_capacity)` owns zero-initialized fixed inline planar storage for exactly one capture producer and one render consumer. A write publishes complete multichannel frames with release ordering after every channel is copied. A read acquires that publication, preserves frame order across wraparound, and zeroes only its unavailable tail. Overflow drops newest frames rather than racing the consumer-owned cursor. Statistics expose current fill plus saturating written, read, dropped, and silent frame totals. All channel, frame, cursor, alias, and overlapping-output validation finishes before publication or caller-output mutation. `updateDriftCorrection` samples the consumer-side fill, rejects a controller target larger than FIFO capacity, and transactionally updates the resampler. Keep the FIFO at a stable address while callbacks run. Stop both callbacks before `reset`; reset clears the complete storage and statistics.

`BoundedCaptureRateBridge(Sample, maximum_channels, fifo_frame_capacity, maximum_capture_chunk, maximum_output_frames)` composes the FIFO, controller, bounded planar scratch, and one synchronized `StreamingResampler` per channel. The capture callback publishes planar blocks through `capture`. The render callback calls `render` with a complete output block. Rendering preflights the proposed correction and exact phase-dependent input demand for every channel, applies one correction, pulls exactly that many FIFO frames in bounded chunks, and produces the complete output span. It neither over-reads capture data nor needs a pending-input buffer.

`CaptureRateLifecycleConfig` keeps startup and recovery priming thresholds, a fixed control-update interval, and underflow and overflow policy caller-owned. A zero control interval preserves one correction update per render callback. A nonzero interval divides arbitrary callback partitions at a stable sample cadence without resetting resampler phase or history. Priming consumes no FIFO data, does not advance the resampler timeline, and emits deterministic silence. Rebuffering after an underflow resets rate state and silences the complete triggering block. Overflow may either continue after dropping newest input or request render-thread recovery, which discards the stale bounded backlog before priming again. `CaptureRateRenderReport` exposes the operating-state transition, discarded frames, real and silent demand, correction, and FIFO fill. Mismatched timelines, malformed retained state, oversized blocks, and aliases fail before output mutation. Unexpected processing failures clear every supplied output. Stop both callbacks before `reset`; reset returns the bridge to its configured startup state.

The zero-valued lifecycle defaults are intentional compatibility behavior: no startup or recovery priming, one correction update per render callback, silence substitution on underflow, and newest-frame rejection without rebuffering on overflow. Report values describe the completed render call and are not accepted as configuration. Unknown policy values are reserved for additive evolution and fail validation before retained state or caller output changes.

`SplitAudioCallback(Sample)` is the native integration boundary for independently timed devices. Its capture function receives only flattened planar input channels. Its render function receives `CaptureRenderBlock`, which carries the render frame count, main and auxiliary outputs, parameter changes, events, output events, and transport. `BoundedCaptureRateCallbackAdapter` binds this split contract to an ordinary `AudioCallback`. It owns corrected planar input storage, restores the configured main and auxiliary input bus boundaries, invokes the existing processor callback on the render thread only while the bridge is running, counts capture and render failures independently, and clears every output during priming or when the device-facing render wrapper rejects a block. Direct `capture` and `render` methods retain checked error and report access for injected backends. Keep the adapter stable while either callback runs and call `reset` only after both stop.
...
`Midi1InputDevice` and `Midi1OutputDevice` use owned three-byte channel messages with nanosecond timestamps. `Midi1InputQueue(capacity)` is a bounded single-producer, single-consumer bridge from the device callback to the audio callback. Producers must submit nondecreasing timestamps. `Midi1BlockScheduler` converts absolute timestamps to floor-rounded sample offsets, clamps late packets to offset zero, and retains future packets or packets that exceed the current event-buffer capacity. Its device callback adapter counts rejected packets atomically. Keep the scheduler at a stable address while the device retains its callback. Stop both callbacks before reset, and do not attach multiple producers or consumers to one scheduler. Queue reset clears every retained packet. `Midi1EventBuffer` converts block-relative device packets into checked plugin events without allocation. Its validity and event view reject a corrupted public count, and `reset` clears the complete event array. The scheduler validates that destination before removing any queued packet. `Midi1OutputSink` converts emitted events back to MIDI 1 and reports sent, unsupported, and rejected messages separately.

`UmpInputDevice` and `UmpOutputDevice` provide the same timestamped device boundary for complete Universal MIDI Packets. `UmpInputQueue(capacity)` and `UmpBlockScheduler(capacity)` preserve 32, 64, 96, and 128-bit packets in fixed storage, enforce nondecreasing producer timestamps, assign floor-rounded sample offsets, clamp late packets to offset zero, and retain future or capacity-limited packets. Quiescent queue reset clears every retained packet. `UmpBlockBuffer(capacity)` deliberately carries raw packets rather than narrowing Utility, System, SysEx, Flex Data, Stream, or MIDI 2 messages into the plugin event model. Its checked `packets` view rejects a corrupted public count, `reset` clears the complete packet array, and the scheduler validates it before consuming queued input. `UmpOutputDevice.sendBlock` converts valid block-relative offsets back to absolute nanosecond timestamps, continues after individual backend rejections, and reports sent, invalid, and rejected packets separately. The same stable-address, one-producer, one-consumer, and stopped-before-reset rules apply. Linux connects these contracts to ALSA UMP RawMIDI. Windows connects them to the optional Windows MIDI Services App SDK backend.

`DeviceCatalog(capacity)` is a control-thread discovery snapshot with bounded UTF-8 identifiers and display names. Identifier, name, and inactive descriptor storage starts at zero. Refresh validates the entire replacement, rejects duplicate identifiers and defaults, clears inactive descriptors, and publishes a new generation only after success. Its borrowed item view revalidates the retained count and every active descriptor, returning empty for malformed copied state. The generation accessor applies the same validation and reports the neutral initial generation zero for malformed state. Unified audio devices and directional audio input or output endpoints are distinct kinds, so platforms with independent capture and render defaults do not need synthetic device pairs. Selection resolves a requested stable identifier first, then the platform default, then the first device of that kind.
...
`DeviceFailureMonitorSet.validate` and `valid` detect a corrupted retained count. Add, replace, and poll reject invalid state without indexing monitor storage. Inactive entries use a safe empty source, and replace or clear scrubs every retired monitor record.
...
`StandaloneWindowBackend` is the platform boundary for a top-level application window. It creates the native parent used by `gui.Editor`, controls visibility and native resize negotiation, supplies bounded window events, and destroys its platform state after editor detachment. Each native backend owns a validated UTF-8 title in zero-initialized fixed storage. `StandaloneWindow` validates size and scale, rolls back a failed editor attachment by closing the native window, propagates resize, scale, and focus events on the control thread, retains quit requests, and tears down each resource once.
...
Parameter-value, parameter-boundary, event-boundary, and combined process iterators expose `valid`. They return no segment without changing their cursor when the retained cursor, frame extent, or borrowed collection is malformed. Parameter and event filter iterators retain the construction frame extent and revalidate the complete borrowed collection before indexing. This rejects invalid post-construction mutation in events, parameter changes, sequences, or ramps without advancing. The filters also validate stored-order indexes and require sample-order offset/index pairs to identify the previously returned matching item. A malformed filter cursor returns no item without changing state.
...
The parser and writer borrow caller storage and do not allocate. Keep source bytes alive and unchanged while using a parsed file or its event slices. Writer operations validate retained positions, active-track headers, header identity, and final bounds before changing storage. Malformed public writer state returns `InvalidMidiWriterState`.
...
The instrument never allocates. Capacity overflow returns `error.MpeNoteCapacityExceeded` without changing existing notes. Inactive note capacity starts in a deterministic state. Note release, sustain release, all-notes-off, `setLayout`, and `reset` scrub vacated note storage. Layout replacement is validated before reset. `valid` and fail-closed accessors contain malformed retained state.
...
The allocator is intentionally a one-note-per-member-channel sender policy. Inactive assignments are deterministic, and release or reset scrubs vacated entries after compaction. The receiver-side instrument also handles MIDI Mode 3 streams where several notes share a channel.

### Universal MIDI Packets

`process.UmpPacket` owns one checked 32, 64, 96, or 128-bit Universal MIDI Packet in fixed storage. Its expected word count comes from the first word's Message Type nibble. `process.UmpIterator` exposes `valid`, revalidates the complete caller-owned word slice, and accepts only cursors at exact packet boundaries. An interior cursor, out-of-range cursor, or truncated final packet is rejected without advancing.
...
`process.Sysex7Packetizer` and `process.Sysex8Packetizer` borrow caller-owned messages and emit complete, begin, continuation, and end packets. Packetization rejects a cursor outside the borrowed message or an empty-message sentinel attached to nonempty input. Their fixed-capacity reassemblers validate packet order, capacity, group continuity, and, for SysEx8, stream identity before changing retained data. Inactive capacity starts at zero, replacement clears a longer prior message, and reset scrubs complete storage. SysEx7 accepts only 7-bit data. SysEx8 preserves all byte values.

`process.MidiStreamMessage` provides checked endpoint discovery and information, device identity, stream configuration, function block discovery and information, and MIDI Clip start and end marker packets. Function block counts are limited to 32, discovery can target one declared block or all blocks, and group ranges must fit the 16 UMP groups. Clip sequencing and the required preceding Delta Clockstamp belong to the caller's MIDI Clip writer. `process.MidiStreamTextPacketizer` emits UTF-8 endpoint and function-block names plus printable-ASCII product instance IDs. It revalidates its bounded borrowed text, notification kind, function-block target, cursor, and empty-message phase before emission. Its fixed-capacity reassembler rejects sequence, notification-kind, function-block, padding, encoding, protocol-length, and storage-capacity mismatches without partial mutation. Inactive capacity is zero, replacement clears the prior tail, and reset scrubs complete storage. Borrowed text and byte views reject malformed retained bytes.
...
`process.MidiEndpointResponder` validates a borrowed endpoint descriptor and answers requester packets through a lazy `MidiEndpointReplies` iterator. Endpoint descriptors include identity, names, the initial configuration, and up to 32 function blocks. The responder accepts supported configuration requests and returns its retained configuration when a request is unsupported. All-block discovery with maximum-length names remains allocation-free and does not build an intermediate packet queue. Reply iteration exposes `valid` and revalidates the borrowed descriptor, retained stream configuration, nested text packetizer, phase coherence, and bounded endpoint and function-block cursors before indexing or advancing. Malformed retained reply state returns `InvalidEndpointRepliesState` without mutation. Keep borrowed descriptor strings and function-block storage unchanged while the responder or its reply iterators use them.
...
`process.MidiCiProfileInquiryTransaction` discovers bounded enabled and disabled Profile lists and enforces the required channel, group, then function-block reply order. `process.MidiCiProfileSetTransaction` correlates Set Profile On or Off messages with Enabled or Disabled reports. Added and Removed reports, Profile Details inquiries, registered channel-count details, and bounded Profile Specific Data cover the optional generic Profile Configuration messages. Profile lists, Details replies, and Specific Data initialize inactive fixed capacity to zero, so construction and parsing do not expose prior storage beyond active borrowed views. Profile-defined targets and payload semantics remain the caller's responsibility.

`process.MidiCiProfileHost` owns a bounded local profile registry. It produces Added and Removed presence reports, answers address-scoped inquiries, publishes local enablement changes, and dispatches incoming enablement, details, and profile-specific-data requests through a typed delegate. Delegate rejection leaves enablement state unchanged. Inactive registry capacity is deterministic, and removal scrubs the vacated entry after compaction. `process.MidiCiDevice.profileHost` borrows the same policy surface while keeping the same deterministic local profile state in the unified device.

`process.MidiCiPropertyCapabilitiesTransaction` and `process.MidiCiPropertyCapabilitiesResponder` negotiate the supported number of simultaneous Property Exchange requests. They cover MIDI-CI message formats 1 and 2, Property Exchange version fields, responder version downgrade, MUID correlation, and the required whole-function-block address. `process.MidiCiPropertyDataMessage` provides bounded wire envelopes for Get, Set, Subscription, their replies, and Notify. It validates request IDs, chunk fields, first-chunk header placement, fixed header-only forms, 7-bit payload bytes, and exact lengths. Inactive header and payload capacity is initialized to zero. `process.MidiCiPropertyReassembler` joins known-length and initially unknown-length chunk sequences, reports aborted transfers, rejects identity and ordering changes, and preserves retained data when a chunk is rejected. Its validation covers lifecycle flags, identities, version, chunk cursor, declared total, fixed-header-only forms, storage bounds, and retained 7-bit bytes. Reset scrubs both retained buffers. Borrowed views return empty and `push` rejects before mutation when retained state is malformed. `process.MidiCiPropertyRequestIds` allocates a negotiated fixed number of concurrent request IDs without heap allocation. It rejects a malformed retained allocation cursor before changing active IDs, while count and membership queries return neutral values until the cursor is repaired.

`process.MidiCiPropertyInitiator` owns a fixed request-ID pool, sequences outgoing chunks, correlates responses by version and both MUIDs, retains completed data until release, and handles timeout-wait, terminate, and timeout notifications. Its validation requires the allocation pool and request slots to agree before mutation or query. `process.MidiCiPropertyResponder` bounds simultaneous remote requests, reassembles each request by remote MUID and request ID, and sequences its reply. Responder validation covers its local identity, active count, request bounds and identities, response cursor state, and duplicate remote request keys. `process.MidiCiPropertySubscriptionRegistry` owns bounded resource and subscription identifiers, correlates partial, full, notify, and end commands, and can release every subscription for a disconnected remote. Its retained entries must preserve the resource and subscription-ID syntax used during registration, with a unique remote and subscription-ID key. Inactive identifier capacity starts at zero, and release scrubs the complete entry. The initiator, responder, and subscription registry expose `validate` and `valid`. Counted mutations reject inconsistent retained counters, while remote-wide release derives the repaired count from occupied slots.
...
`process.MidiCiPropertyRemoteCache` writes a versioned snapshot into caller storage and restores it transactionally. Snapshots retain resource IDs, values, and generation counters. Active entry generations are nonzero and distinct, the next-generation counter cannot name an active entry, and rollover skips every generation still in use. A malformed, truncated, oversized, duplicated, colliding, or trailing entry leaves the live cache unchanged. Inactive key and value capacity starts at zero, and removal scrubs the complete entry. `validate` and `valid` check slot bounds, keys, generations, and retained counts before lookup or mutation. `count` derives its result from occupied slots. `process.MidiCiDevice` exposes the same snapshot size, write, restore, and handle-scoped lookup operations, plus validation of its remote, profile, subscription, cache, and responder counts. Its inactive remote and pending-request capacity is deterministic, and completion, cancellation, and invalidation scrub vacated state.
...
`process.MidiFlexTextPacketizer` supports every standardized Metadata Text and Performance Text status, including project and composition names, credits, recording metadata, lyrics, language tags, and Ruby text. It revalidates its bounded borrowed UTF-8, target, cursor, and empty-message phase before emission and caps messages at the protocol limit of 32 packets. The fixed-capacity reassembler preserves a valid target and text kind across segments, checks packet-count and accumulated-length coherence, rejects retained NUL and leading byte-order-mark data, and changes bytes only after sequence, capacity, and identity checks pass. Inactive capacity is zero, replacement clears the prior tail, and reset scrubs complete storage.

`process.MidiMixedDataPacketizer` emits one checked Mixed Data Set header followed by the required 14-byte payload packets. The header carries group, MDS ID, byte count, chunk position, manufacturer, device, and sub-IDs. Packetization rejects a retained header, source extent, cursor, or header-emission state mismatch before emitting another packet. Its fixed-capacity reassembler derives the final payload length from the header, validates padding and packet identity before copying, and supports header-only chunks. New headers clear prior payload storage, inactive capacity is zero, and reset scrubs complete storage.
