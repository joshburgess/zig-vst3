...
`zig build validate-examples` was also rerun after the list-name helper batch and passed locally against the bundled examples. The local machine has Command Line Tools but not full Xcode, and the validator wrapper now handles that setup.
...
CI checkpoint: after pushing `613c6ef`, GitHub Actions showed run `25708669139` in progress. The latest completed green checkpoint is `03efb99` run `25708514777`. The `7897404` run `25708009016` and `51fa517` run `25707448205` also completed successfully. The intermediate `be54d4c`, `0152e7a`, `46d5999`, `e79b4a3`, `a4f8c22`, `a7cda5a`, `e9b647d`, `b3ffff0`, `f2b00d5`, `0d74831`, `ea6e6ed`, and `3b55ae8` runs were cancelled by newer pushes, which is expected for this workflow. The earlier macOS Layer 1 ABI failure on `bae332c` and `9d8fc4a` was the Zig 0.14.0 native C sanitizer link issue with `libubsan_rt.a`; `f808411` fixed it by disabling C sanitization for the ABI harness executables.
