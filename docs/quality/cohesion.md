...
| `zig-vst3-plugin/src/dsp/adm_xml/core.zig` | 9,880 / 4,786 | Owns bounded ADM model construction, standard iterators, emission-profile checks, and graph validation behind the exact `dsp/adm_xml.zig` facade. Foreign-extension and untyped-metadata traversal now live in `adm_xml/metadata.zig` over a narrow immutable source view. | SPLIT. Continue separating the emission-profile reader from the retained model and graph validator without exposing parser scratch state. |
...
- `zig-vst3-plugin/src/dsp/adm_xml/core.zig` | 14666 | 9880 | 4786 | SPLIT
