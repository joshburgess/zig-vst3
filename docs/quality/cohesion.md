...
| `zig-vst3-plugin/src/dsp/adm_xml/core.zig` | 7,286 / 4,786 | Owns bounded ADM document construction, declaration, reference, profile, tag, and block iterators, and graph validation behind the exact `dsp/adm_xml.zig` facade. Passive values, extension traversal, and emission-profile reading live in focused sibling modules. | SPLIT. Separate the remaining standard metadata iterators from document construction and graph validation without exposing parser scratch state. |
...
- `zig-vst3-plugin/src/dsp/adm_xml/core.zig` | 12072 | 7286 | 4786 | SPLIT
