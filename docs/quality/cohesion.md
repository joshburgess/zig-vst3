...
| `zig-vst3-plugin/src/dsp/mp3.zig` | 12,068 / 12,720 | Owns MPEG Layer III encoding, file writing, and the current public facade. Headers, side information, scale-factor coding, Huffman coding, and passive spectral values live in `mp3/syntax.zig`; audio decode, stream recovery, seeking, and positional reads live in `mp3/decoder.zig`. | SPLIT. Separate reservoir ownership from encoder and file-writing state while preserving exact public identities. |
| `zig-vst3-plugin/src/dsp/mp3/decoder.zig` | 2,879 / 0 | Owns Layer III requantization, stereo and hybrid synthesis, frame and stream decoder state, recovery, seeking, and positional file reads. | SPLIT. Separate the decode and synthesis state from metadata parsing, seeking, and positional reader state. |
...
- `zig-vst3-plugin/src/dsp/mp3.zig` | 24788 | 12068 | 12720 | SPLIT
- `zig-vst3-plugin/src/dsp/mp3/decoder.zig` | 2879 | 2879 | 0 | SPLIT
