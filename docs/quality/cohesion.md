...
| `zig-vst3-plugin/src/dsp/adm_xml/core.zig` | 9,665 / 4,786 | Owns bounded ADM document construction, standard iterators, emission-profile checks, and graph validation behind the exact `dsp/adm_xml.zig` facade. Passive block, gain, position, tag, and exclusion values live in `adm_xml/model.zig`; foreign-extension and untyped-metadata traversal lives in `adm_xml/metadata.zig`. | SPLIT. Continue separating the emission-profile reader from document construction and graph validation without exposing parser scratch state. |
...
- `zig-vst3-plugin/src/dsp/adm_xml/core.zig` | 14451 | 9665 | 4786 | SPLIT
