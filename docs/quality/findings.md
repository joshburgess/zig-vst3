...
| Q-MEM-001 | Medium | Q04 | In progress | Framework VST3 component and controller objects hard-coded `page_allocator` for allocation and destruction. This preserved allocator compatibility in production but prevented deterministic failure injection for the owning object and processor-construction boundary. | Store allocator provenance in each owning object, add an internal allocator-injected creation path while preserving the public ABI, and verify outer and nested allocation failure cleanup. |
