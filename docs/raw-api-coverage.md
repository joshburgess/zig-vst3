...
| Validator | macOS, Linux, and Windows validator runs in public CI (native bundles on macOS and Linux, cross-built bundles on Windows) | Real-host validation coverage as more DAWs are exercised |
| pluginval | Headless wrapper, native example build steps, strictness 10 gate, and required macOS, Linux, and Windows CI jobs | Real-host pluginval runs in actual DAWs as part of host smoke coverage |
