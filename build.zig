...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag == .linux)
        midi_file_fuzz_module.link_libc = true;
    const midi_file_fuzz_tests = b.addTest(.{
        .root_module = midi_file_fuzz_module,
        .filters = &.{"fuzz MIDI file parsing and traversal"},
    });
    const midi_file_fuzz_step = b.step(
        "test-midi-file-fuzz",
        "Run or fuzz the bounded Standard MIDI File parser",
    );
    ...
    const mp3_fuzz_module = b.createModule(.{
        .root_source_file = b.path("zig-vst3-plugin/src/core.zig"),
        .target = target,
        .optimize = optimize,
    });
    if (target.result.os.tag == .linux)
        mp3_fuzz_module.link_libc = true;
    const mp3_fuzz_tests = b.addTest(.{
        .root_module = mp3_fuzz_module,
        .filters = &.{"fuzz bounded MP3 framing and decoding"},
    });
    const mp3_fuzz_step = b.step(
        "test-mp3-fuzz",
        "Run or fuzz bounded MP3 framing and decoding",
    );
    mp3_fuzz_step.dependOn(
        &b.addRunArtifact(mp3_fuzz_tests).step,
    );
    const ogg_fuzz_module = b.createModule(.{
        .root_source_file = b.path("zig-vst3-plugin/src/core.zig"),
        .target = target,
        .optimize = optimize,
    });
    if (target.result.os.tag == .linux)
        ogg_fuzz_module.link_libc = true;
    const ogg_fuzz_tests = b.addTest(.{
        .root_module = ogg_fuzz_module,
        .filters = &.{"fuzz bounded Ogg page and packet parsing"},
    });
    const ogg_fuzz_step = b.step(
        "test-ogg-fuzz",
        "Run or fuzz bounded Ogg page and packet parsing",
    );
    ...
    const audio_metadata_fuzz_module = b.createModule(.{
        .root_source_file = b.path("zig-vst3-plugin/src/core.zig"),
        .target = target,
        .optimize = optimize,
    });
    if (target.result.os.tag == .linux)
        audio_metadata_fuzz_module.link_libc = true;
    const audio_metadata_fuzz_tests = b.addTest(.{
        .root_module = audio_metadata_fuzz_module,
        .filters = &.{"fuzz bounded audio metadata parsing"},
    });
    const audio_metadata_fuzz_step = b.step(
        "test-audio-metadata-fuzz",
        "Run or fuzz bounded audio metadata parsing",
    );
    ...
    if (b.graph.host.result.os.tag == .linux)
        midi_file_benchmark_core.link_libc = true;
    const midi_file_benchmark = b.addExecutable(.{
        .name = "midi-file-complexity",
        .root_module = b.createModule(.{
            .root_source_file = b.path("tools/midi_file_complexity.zig"),
            .target = b.graph.host,
            .optimize = optimize,
            .link_libc = true,
        }),
    });
    midi_file_benchmark.root_module.addImport(
        "zig-vst3-plugin-core",
        midi_file_benchmark_core,
    );
    const midi_file_benchmark_step = b.step(
        "benchmark-midi-file-parser",
        "Measure bounded Standard MIDI File parser scaling",
    );
    ...
    test_step.dependOn(
        &b.addSystemCommand(
            &.{"scripts/check_quality_numerics_inventory.sh"},
        ).step,
    );
    test_step.dependOn(
        &b.addSystemCommand(
    ...
    test_step.dependOn(
        &b.addSystemCommand(
            &.{"scripts/test_quality_cohesion_inventory_runner.sh"},
        ).step,
    );
    test_step.dependOn(
        &b.addSystemCommand(
    ...
    test_step.dependOn(midi_file_benchmark_step);
    test_step.dependOn(
        &b.addSystemCommand(
